...
```js tab=node-cjs title="example.spec.ts"
import { test, expect } from '@playwright/test';
...
```js tab=node-esm title="example.spec.ts"
import { test, expect } from '@playwright/test';
import { join } from 'path';

test('example test', async ({ page }) => {
  await page.goto('https://playwright.dev');
  await expect(page).toHaveScreenshot({ stylePath: join(import.meta.dirname, 'screenshot.css') });
});
```
