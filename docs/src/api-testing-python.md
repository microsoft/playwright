...
These tests assume that repository exists. You probably want to create a new one before running tests and delete it afterwards. Use a [session fixture](https://docs.pytest.org/en/stable/example/special.html) for that. The part before `yield` is the before all and after is the after all.
