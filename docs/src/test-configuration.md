...
- `webServer: { command: string, port?: number, useExistingWebServer?: boolean, cwd?: string, timeout?: number, env?: object }` - Launch a web server before the tests will start. It will automatically detect the port when it got printed to the stdout.
