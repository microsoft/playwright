...
test("configure routing through hooks config", async ({ page, mount }) => {
  ...
  await expect(component.getByRole("link")).toHaveAttribute(
    "href",
    "/products/42"
  );
});
...
import {
  beforeMount,
  afterMount,
} from "@playwright/experimental-ct-solid/hooks";
...
import {
  beforeMount,
  afterMount,
} from "@playwright/experimental-ct-vue2/hooks";
