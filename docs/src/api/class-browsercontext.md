...
## async method: BrowserContext.routeFromCache
* since: v1.62
* langs: js
- returns: <[Disposable]>
...
On the first run every matching request goes to the network and its response (status, headers and body) is stored under [`option: BrowserContext.routeFromCache.dir`]. On later runs matching requests are served from disk without hitting the network. Non-matching requests are left untouched.
...
### param: BrowserContext.routeFromCache.options
* since: v1.62
- `options` <[Object]>
