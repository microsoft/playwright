...
Returns the value of the header matching the name. The name is case insensitive. If multiple headers have the same name, for example `set-cookie`, the they are returned as a list separated by `, `. If no header is found, `null` is returned.
