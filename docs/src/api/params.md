...
- `clientCertificates` <[Array]<[Object]>>
  * alias: ClientCertificate
  ...
  - `sendNone` ?<[boolean]> Explicitly send no client certificate.
...
Client certificate authentication is only active when at least one client certificate is provided. If you want to reject all client certificates sent by the server, you need to set `sendNone` to `true` for the origin you don't want to provide a client certificate for.
