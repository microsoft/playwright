...
container: mcr.microsoft.com/playwright:v1.22.1-focal
...
1. Use the pre-built [Docker image](./docker.md) in your config like so:
   ...
   ```yml
   docker:
     - image: mcr.microsoft.com/playwright:v1.22.1-focal
   environment:
...
```groovy
pipeline {
   agent { docker { image 'mcr.microsoft.com/playwright:v1.22.1-focal' } }
   stages {
      stage('e2e-tests') {
         steps {
            sh 'npm install'
