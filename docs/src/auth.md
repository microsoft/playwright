...
Create a new `setup` project in the config and declare it as a [dependency](./test-projects-js.md#dependencies) for all your testing projects. This project will always run and authenticate before all the tests. All testing projects should use the authenticated state as `storageState`.
