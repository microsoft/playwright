...
[`method: Test.step`] titles that start with `assert` (for example `assert user is logged in`) appear with an **assert** badge in the actions list. Test runner `expect` steps from Playwright Test tracing are shown the same way.
