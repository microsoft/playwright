...
export const test = _baseTest.extend<TestFixtures, WorkerFixtures>({
      ...
      await use(() => {
        if (!dir) {
          dir = path.join(
            ...
            '.playwright-artifacts-' + workerInfo.workerIndex,
          );
          fs.mkdirSync(dir, { recursive: true });
        }
        return dir;
      });
      if (dir) await removeFolders([dir]);
    },
    { scope: 'worker' },
  ],
  ...
  browser: [
    async ({ playwright, browserName, channel, headless, connectOptions }, use) => {
  ...
  _snapshotSuffix: [
    process.env.PLAYWRIGHT_DOCKER ? 'docker' : process.platform,
    { scope: 'worker' },
  ],
  ...
  _setupContextOptionsAndArtifacts: [
    async (
      ...
      if (debugMode()) testInfo.setTimeout(0);

      let traceMode = typeof trace === 'string' ? trace : trace.mode;
      if ((traceMode as any) === 'retry-with-trace') traceMode = 'on-first-retry';
      ...
      const createInstrumentationListener = (context?: BrowserContext) => {
        return {
          onApiCallBegin: (apiCall: string, stackTrace: ParsedStackTrace | null, userData: any) => {
            if (apiCall.startsWith('expect.')) return { userObject: null };
      ...
      const startTracing = async (tracing: Tracing) => {
        if (captureTrace) {
          const title = [
            path.relative(testInfo.project.testDir, testInfo.file) + ':' + testInfo.line,
      ...
      const onDidCreateBrowserContext = async (context: BrowserContext) => {
        ...
        context.setDefaultNavigationTimeout(
          testInfo.timeout === 0 ? 0 : navigationTimeout || actionTimeout || 0,
        );
      ...
      }
      {
        ...
        const existingApiRequests: APIRequestContext[] = Array.from(
          (playwright.request as any)._contexts as Set<APIRequestContext>,
        );
      ...
      const preserveTrace =
        captureTrace &&
        (traceMode === 'on' ||
          (testFailed && traceMode === 'retain-on-failure') ||
          (traceMode === 'on-first-retry' && testInfo.retry === 1));
      const captureScreenshots =
        screenshot === 'on' || (screenshot === 'only-on-failure' && testFailed);

      const traceAttachments: string[] = [];
      const addTraceAttachment = () => {
        const tracePath = testInfo.outputPath(
          `trace${traceAttachments.length ? '-' + traceAttachments.length : ''}.zip`,
        );
      ...
      const addScreenshotAttachment = () => {
        const screenshotPath = testInfo.outputPath(
          `test-${testFailed ? 'failed' : 'finished'}-${screenshotAttachments.length + 1}.png`,
        );
      ...
      const leftoverApiRequests: APIRequestContext[] = Array.from(
        (playwright.request as any)._contexts as Set<APIRequestContext>,
      );
      ...
      const stopTraceChunk = async (tracing: Tracing): Promise<boolean> => {
        ...
        // for the second time.
        if ((tracing as any)[startedCollectingArtifacts]) return false;
        if (preserveTrace) await tracing.stopChunk({ path: addTraceAttachment() });
            ...
            if (captureScreenshots)
              await Promise.all(
                context
                  ...
                  .map((page) =>
                    page
                      .screenshot({ timeout: 5000, path: addScreenshotAttachment() })
                      .catch(() => {}),
                  ),
              );
          })
          .concat(
            leftoverApiRequests.map(async (context) => {
              const tracing = (context as any)._tracing as Tracing;
      ...
      await Promise.all(
        temporaryTraceFiles.map(async (file) => {
          if (preserveTrace) await fs.promises.rename(file, addTraceAttachment()).catch(() => {});
          else await fs.promises.unlink(file).catch(() => {});
        }),
      );
      await Promise.all(
        temporaryScreenshots.map(async (file) => {
          if (captureScreenshots)
      ...
      if (hookType(testInfo) === 'beforeAll' && testInfo.status === 'timedOut') {
        ...
        const pendingCalls = anyContext
          ? formatPendingCalls((anyContext as any)._connection.pendingProtocolCalls())
          : '';
        await Promise.all(
          leftoverContexts.filter((c) => createdContexts.has(c)).map((c) => c.close()),
        );
        if (pendingCalls) testInfo.error = prependToTestError(testInfo.error, pendingCalls);
      }
    },
    { auto: true },
  ],
  ...
  _contextFactory: async ({ browser, video, _artifactsDir }, use, testInfo) => {
    ...
    await use(async (options) => {
      ...
      if (hook)
        throw new Error(
          `"context" and "page" fixtures are not supported in ${hook}. Use browser.newContext() instead.`,
        );
    ...
    if (prependToError) testInfo.error = prependToTestError(testInfo.error, prependToError);
  },
...
function formatPendingCalls(calls: ParsedStackTrace[]) {
  ...
  return (
    ...
    calls
      .map((call) => {
        const frame =
          call.frames && call.frames[0] ? ' at ' + formatStackFrame(call.frames[0]) : '';
