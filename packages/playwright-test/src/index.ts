...
import { createGuid, debugMode, addInternalStackPrefix, mergeTraceFiles, saveTraceFile, removeFolders, isString, asLocator, jsonStringifyEncodeUnicode } from 'playwright-core/lib/utils';
    ...
    if (connectOptions && process.env.PW_TEST_REUSE_CONTEXT) {
      const browser = await playwright[browserName].connect({
        ...
        headers: {
          ...
          'x-playwright-launch-options': jsonStringifyEncodeUnicode(_browserOptions),
