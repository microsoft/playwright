...
export class TestTypeImpl {
  ...
  private _createTest(
    ...
    const test = new TestCase(
      ...
      title,
      fn,
      nextOrdinalInFile(suite._requireFile),
      this,
      location,
    );
  ...
  private _describe(
    ...
    for (let parent: Suite | undefined = suite; parent; parent = parent.parent) {
      ...
      if (parent._parallelMode === 'parallel' && child._parallelMode === 'serial')
        throw errorWithLocation(
          location,
          'describe.serial cannot be nested inside describe.parallel',
        );
    }
  ...
  private _configure(location: Location, options: { mode?: 'parallel' | 'serial' }) {
    ...
    if (suite._parallelMode !== 'default')
      throw errorWithLocation(
        location,
        'Parallel mode is already assigned for the enclosing scope.',
      );
  ...
  private _modifier(
    ...
    location: Location,
    ...modifierArgs: [arg?: any | Function, description?: string]
  ) {
    const suite = currentlyLoadingFileSuite();
    if (suite) {
      if (
  ...
  private async _step(location: Location, title: string, body: () => Promise<void>): Promise<void> {
    const testInfo = currentTestInfo();
    if (!testInfo) throw errorWithLocation(location, `test.step() can only be called from a test`);
