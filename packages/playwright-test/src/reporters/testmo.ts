...
class TestmoReporter extends EmptyReporter {
  ...
  private includeSteps = false;

  constructor(options: { outputFile?: string; stripANSIControlSequences?: boolean, includeSteps?: boolean } = {}) {
    super();
  ...
  override async onEnd(result: FullResult) {
    ...
    const root: XMLEntry = {
      name: 'testsuites',
      attributes: {
        id: process.env[`PLAYWRIGHT_TESTMO_SUITE_ID`] || '',
        name: process.env[`PLAYWRIGHT_TESTMO_SUITE_NAME`] || '',
  ...
  private _addTestCase(suiteName: string, test: TestCase, entries: XMLEntry[]) {
      ...
      for (const result of test.results) {
        for (const step of result.steps) {
          ...
          const property: XMLEntry = {
            name: 'property',
            attributes: {
              name: `step[${stepStatus}]`,
              value: step.title
            },
            text: step.error ? stripAnsiEscapes(formatFailure(this.config, test).message) : undefined
          };
          properties.children?.push(property);
        }
      }
    ...
    if (!test.ok()) {
      entry.children.push({
        name: 'failure',
        attributes: {
          message: `${path.basename(test.location.file)}:${
            test.location.line
          }:${test.location.column} ${test.title}`,
          type: 'FAILURE',
        },
...
function serializeXML(
  ...
  const attrs: string[] = [];
  const isHtmlAttribute = typeof(entry.attributes?.name) === 'string' && entry.attributes.name.startsWith('html:');
  ...
  if (entry.text) {
    if (isHtmlAttribute)
      tokens.push(escape(entry.text, false, true));
    else
...
function reportOutputNameFromEnv(): string | undefined {
  if (process.env[`PLAYWRIGHT_TESTMO_OUTPUT_NAME`])
    return path.resolve(process.cwd(), process.env[`PLAYWRIGHT_TESTMO_OUTPUT_NAME`]);
  return undefined;
}
