...
export interface Page {
   ...
   * [page.addOverlayHandler(locator, handler)](https://playwright.dev/docs/api/class-page#page-add-overlay-handler)
   * allows handling an overlay during an action that it would block.
   *
   * This method registers a handler for an overlay that is executed once the overlay locator is visible on the page.
   * The handler should get rid of the overlay so that actions blocked by it can proceed. This is useful for
   ...
   *
   * **NOTE** Running the overlay handler will alter your page state mid-test. For example it will change the currently
   * focused element and move the mouse. Make sure that the actions that run after the overlay handler are
   * self-contained and do not rely on the focus and mouse state. <br /> <br /> For example, consider a test that calls
   ...
   * @param handler Function that should be run once `locator` is visible. This function should get rid of the overlay that blocks
   * actions.
   */
  addOverlayHandler(locator: Locator, handler: Function): Promise<void>;
