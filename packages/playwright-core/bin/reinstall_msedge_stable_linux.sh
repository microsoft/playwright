  ...
  case "${ID}" in
    ...
    *)
      if [[ "${ID_LIKE}" == *debian* ]]; then
        ...
        echo "ERROR: 'npx playwright install msedge' only supports distributions with an official"
        ...
        case "${ID} ${ID_LIKE}" in
          *arch*|*cachyos*)
            echo "On Arch-based systems install Microsoft Edge from the AUR ('yay -S microsoft-edge-stable-bin')."
            ;;
          *)
...
REPO
  ...
  # 2. make sure to remove old stable if any.
  if rpm -q microsoft-edge-stable >/dev/null 2>&1; then
    "${PKG_REMOVE[@]}" microsoft-edge-stable
  fi
