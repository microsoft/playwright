...
export const Workbench: React.FunctionComponent<{}> = () => {
  ...
  React.useEffect(() => {
    ...
    if (!newTraceURLs.some((url) => url.startsWith('blob:'))) setTraceURLs(newTraceURLs);
  }, [setTraceURLs]);
    ...
    (async () => {
      if (traceURLs.length) {
        ...
        for (let i = 0; i < traceURLs.length; i++) {
          ...
          if (uploadedTraceNames.length) params.set('traceFileName', uploadedTraceNames[i]);
  ...
  const { errors, warnings } = selectedAction
  ...
  const tabs = [
    { id: 'logs', title: 'Call', count: 0, render: () => <CallTab action={selectedAction} /> },
    {
      ...
      title: 'Console',
      count: consoleCount,
      render: () => <ConsoleTab action={selectedAction} />,
    },
    {
      id: 'network',
      title: 'Network',
      count: networkCount,
      render: () => <NetworkTab action={selectedAction} />,
    },
  ];
  ...
  if (model.hasSource)
    tabs.push({
      ...
      title: 'Source',
      count: 0,
      render: () => <SourceTab action={selectedAction} />,
    });
                ...
                <div className="vbox">
                  ...
                  {model.wallTime && (
                    <div className="call-line">
                      start time:{' '}
                      <span className="datetime" title={new Date(model.wallTime).toLocaleString()}>
                  ...
                  <div className="call-line">
                    duration:{' '}
                    <span className="number" title={msToString(model.endTime - model.startTime)}>
                  ...
                  <div className="call-line">
                    engine:{' '}
                    <span className="string" title={model.browserName}>
                      {model.browserName}
                    </span>
                  </div>
                  {model.platform && (
                    <div className="call-line">
                      platform:{' '}
                      <span className="string" title={model.platform}>
                        {model.platform}
                      </span>
                    </div>
                  )}
                  {model.options.userAgent && (
                    <div className="call-line">
                      user agent:{' '}
                      <span className="datetime" title={model.options.userAgent}>
                        {model.options.userAgent}
                      </span>
                    </div>
                  )}
                  ...
                  {model.options.viewport && (
                    <div className="call-line">
                      width:{' '}
                      <span className="number" title={String(!!model.options.viewport?.width)}>
                  ...
                  {model.options.viewport && (
                    <div className="call-line">
                      height:{' '}
                      <span className="number" title={String(!!model.options.viewport?.height)}>
                  ...
                  <div className="call-line">
                    is mobile:{' '}
                    <span className="boolean" title={String(!!model.options.isMobile)}>
                  ...
                  {model.options.deviceScaleFactor && (
                    <div className="call-line">
                      device scale:{' '}
                      <span className="number" title={String(model.options.deviceScaleFactor)}>
                  ...
                  <div className="call-line">
                    pages: <span className="number">{model.pages.length}</span>
                  </div>
                  <div className="call-line">
                    actions: <span className="number">{model.actions.length}</span>
                  </div>
                  <div className="call-line">
                    events: <span className="number">{model.events.length}</span>
                  </div>
                </div>
              ),
            },
      ...
      {fileForLocalModeError && (
        <div className="drop-target">
          ...
          <div style={{ paddingTop: 20 }}>
            <div>
              1. Click <a href={fileForLocalModeError}>here</a> to put your trace into the download
              shelf
            </div>
            <div>
