...
export const CallTab: React.FunctionComponent<{
  ...
  const duration = action.metadata.endTime
    ? msToString(action.metadata.endTime - action.metadata.startTime)
    : 'Timed Out';
  return (
        ...
        <>
          ...
          {action.metadata.wallTime && (
            <div className="call-line">
              wall time:{' '}
              <span className="datetime" title={wallTime}>
                {wallTime}
              </span>
            </div>
          )}
          <div className="call-line">
            duration:{' '}
            <span className="datetime" title={duration}>
              {duration}
            </span>
          </div>
        </>
...
function renderLine(metadata: CallMetadata, name: string, value: any, key: string) {
  ...
  return (
    <div key={key} className="call-line">
      {name}:{' '}
      <span className={type} title={title}>
        {text}
      </span>
    </div>
  );
...
function toString(
  ...
  if (type !== 'object' || value === null) return { title: String(value), type };
  if (value.guid) return { title: '<handle>', type: 'handle' };
