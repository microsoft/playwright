...
export class Page extends ChannelOwner<channels.PageChannel> implements api.Page {
  ...
  private _overlayHandlers = new Map<number, Function>();
  ...
  constructor(parent: ChannelOwner, type: string, guid: string, initializer: channels.PageInitializer) {
    ...
    this._channel.on('overlayHandlerTriggered', ({ uid }) => this._onOverlayHandlerTriggered(uid));
  ...
  async addOverlayHandler(locator: Locator, handler: Function): Promise<void> {
    if (locator._frame !== this._mainFrame)
      throw new Error(`Locator must belong to the main frame of this page`);
    const { uid } = await this._channel.registerOverlayHandler({ selector: locator._selector });
    this._overlayHandlers.set(uid, handler);
  }

  private async _onOverlayHandlerTriggered(uid: number) {
    try {
      const handler = this._overlayHandlers.get(uid);
      await handler?.();
    } finally {
      this._wrapApiCall(() => this._channel.resolveOverlayHandlerNoReply({ uid }), true).catch(() => {});
    }
  }
