...
export class Page extends ChannelOwner<channels.PageChannel> implements api.Page {
  ...
  async agent(options: Parameters<api.Page['agent']>[0] = {}) {
    const params: channels.PageAgentParams = {
      ...
      apiCacheFile: (options.provider as any)?._apiCacheFile ? this._platform.resolve((options.provider as any)._apiCacheFile) : undefined,
      doNotRenderActive: (options as any)._doNotRenderActive,
      model: options.provider?.model,
      cacheFile: options.cache?.cacheFile ? this._platform.resolve(options.cache.cacheFile) : undefined,
      cacheOutFile: options.cache?.cacheOutFile ? this._platform.resolve(options.cache.cacheOutFile) : undefined,
