...
import { jsonStringifyEncodeUnicode, assert, headersObjectToArray, monotonicTime } from '../utils';
...
export class BrowserType extends ChannelOwner<channels.BrowserTypeChannel> implements api.BrowserType {
  ...
  private async _connectInsteadOfLaunching(connectOptions: ConnectOptions, launchOptions: LaunchOptions): Promise<Browser> {
    return this._connect({
      ...
      headers: {
        'x-playwright-launch-options': jsonStringifyEncodeUnicode({ ...this._defaultLaunchOptions, ...launchOptions }),
