...
export class DashboardConnection implements Transport {
  ...
  revealPage(pageId_: string) {
    this._pendingReveal = { pageId: pageId_ };
    this._tryRevealPending();
  }

  private _tryRevealPending() {
    const pending = this._pendingReveal;
    if (!pending)
      return;
    for (const { context, descriptor } of this._provider.contextEntries()) {
      let page: api.Page | undefined;
      if (pending.pageId !== undefined)
        page = context.pages().find(p => pageId(p) === pending.pageId);
      else if (pending.sessionName !== undefined && descriptor.title === pending.sessionName
          && (pending.workspaceDir === undefined || descriptor.workspaceDir === pending.workspaceDir))
