...
export function decorateMCPCommand(command: Command) {
  command
      ...
      .option('--auth-token <token>', 'pre-shared bearer token required on every HTTP request to the MCP endpoints. Strongly recommended when binding the server to a non-loopback host (e.g. 0.0.0.0). Prefer the PLAYWRIGHT_MCP_AUTH_TOKEN env var so the token is not visible in process listings.')
