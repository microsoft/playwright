...
program
  ...
  .action(async function (args: string[], options: { dryRun?: boolean }) {
    try {
      if (!args.length) await registry.installDeps(registry.defaultExecutables(), !!options.dryRun);
      else await registry.installDeps(checkBrowsersToInstall(args), !!options.dryRun);
    } catch (e) {
...
program.command('run-driver', { hidden: true }).action(function (options) {
  runDriver();
});
...
program.command('print-api-json', { hidden: true }).action(function (options) {
  printApiJson();
});
...
async function launchContext(
  ...
  if (options.channel) launchOptions.channel = options.channel as any;
  ...
  async function closeBrowser() {
    ...
    closingBrowser = true;
    if (options.saveTrace) await context.tracing.stop({ path: options.saveTrace });
  ...
  if (options.saveTrace) await context.tracing.start({ screenshots: true, snapshots: true });
...
async function launchGridServer(
  factoryPathOrPackageName: string,
  port: number,
