...
class Connection {
  ...
  constructor(ws: WebSocket, request: http.IncomingMessage, enableSocksProxy: boolean, browser: Browser | undefined, onClose: () => void) {
    ...
    new Root(this._dispatcherConnection, async (scope, params) => {
      if (browser)
      ...
      if (!browserAlias)
        return await this._initPlaywrightConnectMode(scope, enableSocksProxy && !!params.enableSocksProxy);
      return await this._initLaunchBrowserMode(scope, browserAlias, enableSocksProxy && !!params.enableSocksProxy);
    });
  }
  ...
  private async _initLaunchBrowserMode(scope: DispatcherScope, browserAlias: string, enableSocksProxy: boolean) {
