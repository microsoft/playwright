...
function launchGridBrowserWorker(gridURL: string, agentId: string, workerId: string, browserName: string) {
  ...
  new PlaywrightConnection(Promise.resolve(), 'auto', ws, false, { enableSocksProxy: true, browserName, launchOptions: {} }, { playwright: null, browser: null, android: null }, log, async () => {
    log('exiting process');
