...
export class WebKit extends BrowserType {
  ...
  override amendEnvironment(env: Env, userDataDir: string, executable: string, browserArguments: string[], channel?: string): Env {
    return { ...env, CURL_COOKIE_JAR_PATH: path.join(channel === 'webkit-wsl' ? translatePathToWSL(userDataDir) : userDataDir, 'cookiejar.db'), };
  }
...
export function translatePathToWSL(path: string): string {
  return spawnSync('wsl', ['-d', 'playwright', 'wslpath', path.replace(/\\/g, '\\\\')]).stdout.toString().trim()
}
