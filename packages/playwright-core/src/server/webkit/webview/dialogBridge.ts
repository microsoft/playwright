...
// Stock WebKit RDP has no dialog API, so the WebView backend overrides
// window.alert/confirm/prompt at bootstrap time and tunnels the call through
// a synchronous XHR to this HTTP server. The server holds the response until
// the host-side Dialog handler (test or auto-dismiss) calls back.
