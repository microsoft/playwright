...
export class WKBrowser extends Browser {
  ...
  static async connect(
    ...
    const promises: Promise<any>[] = [browser._browserSession.send('Playwright.enable')];
  ...
  constructor(transport: ConnectionTransport, options: BrowserOptions) {
    super(options);
    this._connection = new WKConnection(
      transport,
  ...
  _onPageProxyCreated(event: Protocol.Playwright.pageProxyCreatedPayload) {
    ...
    if (!context) context = this._defaultContext as WKBrowserContext;
    if (!context) return;
    const pageProxySession = new WKSession(
      this._connection,
      pageProxyId,
      `Target closed`,
      (message: any) => {
...
export class WKBrowserContext extends BrowserContext {
  ...
  async _doCookies(urls: string[]): Promise<types.NetworkCookie[]> {
    ...
    return network.filterCookies(
      cookies.map((c: types.NetworkCookie) => {
        const copy: any = { ...c };
  ...
  async _doGrantPermissions(origin: string, permissions: string[]) {
    await Promise.all(
      this.pages().map((page) => (page._delegate as WKPage)._grantPermissions(origin, permissions)),
    );
  }
  ...
  async setExtraHTTPHeaders(headers: types.HeadersArray): Promise<void> {
    ...
    for (const page of this.pages()) await (page._delegate as WKPage).updateExtraHTTPHeaders();
  }

  async setOffline(offline: boolean): Promise<void> {
    this._options.offline = offline;
    for (const page of this.pages()) await (page._delegate as WKPage).updateOffline();
  }
  ...
  async _doUpdateRequestInterception(): Promise<void> {
    for (const page of this.pages()) await (page._delegate as WKPage).updateRequestInterception();
  }
