...
export class WKPage implements PageDelegate {
  ...
  private readonly _requestIdToResponseReceivedPayloadEvent = new Map<
    string,
    Protocol.Network.responseReceivedPayload
  >();
  ...
  constructor(
    browserContext: WKBrowserContext,
    pageProxySession: WKSession,
    opener: WKPage | null,
  ) {
    ...
    this._page.on(Page.Events.FrameDetached, (frame: frames.Frame) =>
    ...
    if (
      ...
      const viewportSize = helper.getViewportSizeFromWindowFeatures(
        opener._nextWindowOpenPopupFeatures,
      );
  ...
  async _initializeSession(
    session: WKSession,
    provisional: boolean,
    resourceTreeHandler: (r: Protocol.Page.getResourceTreeReturnValue) => void,
  ) {
  ...
  private async _initializeSessionMayThrow(
    session: WKSession,
    resourceTreeHandler: (r: Protocol.Page.getResourceTreeReturnValue) => void,
  ) {
    ...
    this._page
      .frames()
      .map((frame) => frame.evaluateExpression(bootstrapScript, false, undefined).catch((e) => {}));
    ...
    promises.push(
      session.send('Network.setExtraHTTPHeaders', {
        headers: headersArrayToObject(this._calculateExtraHTTPHeaders(), false /* lowerCase */),
      }),
    );
    ...
    promises.push(
      session.send('Page.overrideSetting', {
        setting: 'DeviceOrientationEventEnabled' as any,
        value: contextOptions.isMobile,
      }),
    );
    promises.push(
      session.send('Page.overrideSetting', {
        setting: 'FullScreenEnabled' as any,
        value: !contextOptions.isMobile,
      }),
    );
    promises.push(
      session.send('Page.overrideSetting', {
        setting: 'NotificationsEnabled' as any,
        value: !contextOptions.isMobile,
      }),
    );
    promises.push(
      session.send('Page.overrideSetting', {
        setting: 'PointerLockEnabled' as any,
        value: !contextOptions.isMobile,
      }),
    );
    promises.push(
      session.send('Page.overrideSetting', {
        setting: 'InputTypeMonthEnabled' as any,
        value: contextOptions.isMobile,
      }),
    );
    promises.push(
      session.send('Page.overrideSetting', {
        setting: 'InputTypeWeekEnabled' as any,
        value: contextOptions.isMobile,
      }),
    );
    await Promise.all(promises);
  ...
  private _addSessionListeners() {
    this._sessionListeners = [
      eventsHelper.addEventListener(this._session, 'Page.frameNavigated', (event) =>
        this._onFrameNavigated(event.frame, false),
      ),
      eventsHelper.addEventListener(this._session, 'Page.navigatedWithinDocument', (event) =>
        this._onFrameNavigatedWithinDocument(event.frameId, event.url),
      ),
      eventsHelper.addEventListener(this._session, 'Page.frameAttached', (event) =>
        this._onFrameAttached(event.frameId, event.parentFrameId),
      ),
      eventsHelper.addEventListener(this._session, 'Page.frameDetached', (event) =>
        this._onFrameDetached(event.frameId),
      ),
      eventsHelper.addEventListener(this._session, 'Page.willCheckNavigationPolicy', (event) =>
        this._onWillCheckNavigationPolicy(event.frameId),
      ),
      eventsHelper.addEventListener(this._session, 'Page.didCheckNavigationPolicy', (event) =>
        this._onDidCheckNavigationPolicy(event.frameId, event.cancel),
      ),
      eventsHelper.addEventListener(this._session, 'Page.frameScheduledNavigation', (event) =>
        this._onFrameScheduledNavigation(event.frameId),
      ),
      eventsHelper.addEventListener(this._session, 'Page.frameStoppedLoading', (event) =>
        this._onFrameStoppedLoading(event.frameId),
      ),
      eventsHelper.addEventListener(this._session, 'Page.loadEventFired', (event) =>
        this._onLifecycleEvent(event.frameId, 'load'),
      ),
      eventsHelper.addEventListener(this._session, 'Page.domContentEventFired', (event) =>
        this._onLifecycleEvent(event.frameId, 'domcontentloaded'),
      ),
      eventsHelper.addEventListener(this._session, 'Runtime.executionContextCreated', (event) =>
        this._onExecutionContextCreated(event.context),
      ),
      eventsHelper.addEventListener(this._session, 'Console.messageAdded', (event) =>
        this._onConsoleMessage(event),
      ),
      eventsHelper.addEventListener(this._session, 'Console.messageRepeatCountUpdated', (event) =>
        this._onConsoleRepeatCountUpdated(event),
      ),
      eventsHelper.addEventListener(
        this._pageProxySession,
      ...
      eventsHelper.addEventListener(this._session, 'Page.fileChooserOpened', (event) =>
        this._onFileChooserOpened(event),
      ),
      eventsHelper.addEventListener(this._session, 'Network.requestWillBeSent', (e) =>
        this._onRequestWillBeSent(this._session, e),
      ),
      eventsHelper.addEventListener(this._session, 'Network.requestIntercepted', (e) =>
        this._onRequestIntercepted(this._session, e),
      ),
      eventsHelper.addEventListener(this._session, 'Network.responseReceived', (e) =>
        this._onResponseReceived(e),
      ),
      eventsHelper.addEventListener(this._session, 'Network.loadingFinished', (e) =>
        this._onLoadingFinished(e),
      ),
      eventsHelper.addEventListener(this._session, 'Network.loadingFailed', (e) =>
        this._onLoadingFailed(e),
      ),
      eventsHelper.addEventListener(this._session, 'Network.webSocketCreated', (e) =>
      ...
      eventsHelper.addEventListener(this._session, 'Network.webSocketClosed', (e) =>
        this._page._frameManager.webSocketClosed(e.requestId),
      ),
      eventsHelper.addEventListener(this._session, 'Network.webSocketFrameError', (e) =>
      ...
      if (event.message.stackTrace) {
        stack =
          ...
          event.message.stackTrace
            .map((callFrame) => {
              return `    at ${callFrame.functionName || 'unknown'} (${callFrame.url}:${
  ...
  private async _onFileChooserOpened(event: {
    frameId: Protocol.Network.FrameId;
    element: Protocol.Runtime.RemoteObject;
  }) {
    let handle;
    try {
  ...
  private static async _setEmulateMedia(
    session: WKSession,
  ...
  async updateEmulateMedia(): Promise<void> {
    ...
    await this._forAllSessions((session) =>
      WKPage._setEmulateMedia(session, this._page._state.mediaType, colorScheme, reducedMotion),
    );
  }
  ...
  private async _evaluateBindingScript(binding: PageBinding): Promise<void> {
    ...
    await Promise.all(
      this._page
        .frames()
        .map((frame) => frame.evaluateExpression(script, false, {}).catch((e) => {})),
    );
  }
  ...
  _onRequestWillBeSent(session: WKSession, event: Protocol.Network.requestWillBeSentPayload) {
    ...
    const request = new WKInterceptableRequest(
      session,
      route,
      frame,
      event,
  ...
  private _handleRequestRedirect(
    request: WKInterceptableRequest,
    ...
    response._requestFinished(
      responsePayload.timing ? helper.secondsToRoundishMillis(timestamp - request._timestamp) : -1,
    );
  ...
  async _grantPermissions(origin: string, permissions: string[]) {
    const webPermissionToProtocol = new Map<string, string>([['geolocation', 'geolocation']]);
