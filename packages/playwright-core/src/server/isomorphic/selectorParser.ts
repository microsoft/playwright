    ...
    if (kNestedSelectorNames.has(part.name)) {
      ...
      try {
        ...
        if (!Array.isArray(unescaped) || unescaped.length < 1 || unescaped.length > 2 || typeof unescaped[0] !== 'string')
          throw new Error(`pw3003: Malformed selector: ${part.name}=` + part.body);
      ...
      if (result.body.parsed.parts.some(part => part.name === 'internal:control' && part.body === 'enter-frame'))
        throw new Error(`pw3003: Frames are not allowed inside "${part.name}" selectors`);
      return result;
    }
    return { ...part, source: part.body };
  });
  if (kNestedSelectorNames.has(parts[0].name))
    throw new Error(`pw3003: "${parts[0].name}" selector cannot be first`);
  return {
...
export function parseAttributeSelector(selector: string, allowUnquotedStrings: boolean): AttributeSelector {
  ...
  const syntaxError = (stage: string|undefined) => {
    if (EOL)
      throw new Error(`pw3003: Unexpected end of selector while parsing selector \`${selector}\``);
    throw new Error(`pw3003: Error while parsing selector \`${selector}\` - unexpected symbol "${next()}" at position ${wp}` + (stage ? ' during ' + stage : ''));
  };
  ...
  function readRegularExpression() {
    ...
    try {
      ...
      throw new Error(`pw3003: Error while parsing selector \`${selector}\`: ${e.message}`);
    }
  }
  ...
  function readAttribute(): AttributeSelectorPart {
    ...
    if (next() === '/') {
      if (operator !== '=')
        throw new Error(`pw3003: Error while parsing selector \`${selector}\` - cannot use ${operator} in attribute with regular expression`);
    ...
    if (operator !== '=' && typeof value !== 'string')
      throw new Error(`pw3003: Error while parsing selector \`${selector}\` - cannot use ${operator} in attribute with non-string matching value - ${value}`);
  ...
  if (!result.name && !result.attributes.length)
    throw new Error(`pw3003: Error while parsing selector \`${selector}\` - selector cannot be empty`);
  return result;
}
