...
export class HarTracer {
  ...
  start() {
    ...
    this._started = true;
    const apiRequest =
      this._context instanceof APIRequestContext ? this._context : this._context.fetchRequest;
    this._eventListeners = [
      eventsHelper.addEventListener(
        apiRequest,
        APIRequestContext.Events.Request,
        (event: APIRequestEvent) => this._onAPIRequest(event),
      ),
      eventsHelper.addEventListener(
        apiRequest,
        APIRequestContext.Events.RequestFinished,
        (event: APIRequestFinishedEvent) => this._onAPIRequestFinished(event),
      ),
    ];
    if (this._context instanceof BrowserContext) {
      this._eventListeners.push(
        ...
        eventsHelper.addEventListener(
          ...
          (request: network.Request) => this._onRequest(request),
        ),
        eventsHelper.addEventListener(
          this._context,
        ...
        eventsHelper.addEventListener(
          ...
          (response: network.Response) => this._onResponse(response),
        ),
      );
    }
  }
  ...
  private _onAPIRequest(event: APIRequestEvent) {
    ...
    harEntry.request.postData = postDataForBuffer(
      event.postData || null,
  ...
  private _onResponse(response: network.Response) {
    ...
    const dns =
      timing.domainLookupEnd !== -1
        ? helper.millisToRoundishMillis(timing.domainLookupEnd - timing.domainLookupStart)
        : -1;
    const connect =
      timing.connectEnd !== -1
        ? helper.millisToRoundishMillis(timing.connectEnd - timing.connectStart)
        : -1;
    const ssl =
      timing.connectEnd !== -1
        ? helper.millisToRoundishMillis(timing.connectEnd - timing.secureConnectionStart)
        : -1;
    const wait =
      timing.responseStart !== -1
        ? helper.millisToRoundishMillis(timing.responseStart - timing.requestStart)
        : -1;
    const receive =
      response.request()._responseEndTiming !== -1
        ? helper.millisToRoundishMillis(
