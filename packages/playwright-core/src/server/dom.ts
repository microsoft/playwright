...
export class ElementHandle<T extends Node = Node> extends js.JSHandle<T> {
  ...
  async _retryAction(progress: Progress, actionName: string, action: (retry: number) => Promise<PerformActionResult>, options: { trial?: boolean, force?: boolean, skipOverlayHandlersCheckpoint?: boolean }): Promise<'error:notconnected' | 'done'> {
    let retry = 0;
    ...
    while (progress.isRunning()) {
      ...
      if (!options.skipOverlayHandlersCheckpoint && !options.force)
        await this._frame._page.performOverlayHandlersCheckpoint(progress);
  ...
  async _retryPointerAction(progress: Progress, actionName: ActionName, waitForEnabled: boolean, action: (point: types.Point) => Promise<void>,
    ...
    const skipOverlayHandlersCheckpoint = actionName === 'move and up';
    ...
    }, { ...options, skipOverlayHandlersCheckpoint });
  }
