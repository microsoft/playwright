...
export class FrameExecutionContext extends js.ExecutionContext {
  ...
  constructor(
    delegate: js.ExecutionContextDelegate,
    frame: frames.Frame,
    world: types.World | null,
  ) {
  ...
  async evaluateHandle<Arg, R>(
    pageFunction: js.Func1<Arg, R>,
    arg?: Arg,
...
export class ElementHandle<T extends Node = Node> extends js.JSHandle<T> {
  ...
  async evaluateInUtility<R, Arg>(
    pageFunction: js.Func1<[js.JSHandle<InjectedScript>, ElementHandle<T>, Arg], R>,
    arg: Arg,
  ): Promise<R | 'error:notconnected'> {
    try {
  ...
  async evaluateHandleInUtility<R, Arg>(
    pageFunction: js.Func1<[js.JSHandle<InjectedScript>, ElementHandle<T>, Arg], R>,
    arg: Arg,
  ): Promise<js.JSHandle<R> | 'error:notconnected'> {
    try {
      const utility = await this._frame._utilityContext();
  ...
  async evaluatePoll<R, Arg>(
    progress: Progress,
    pageFunction: js.Func1<
      [js.JSHandle<InjectedScript>, ElementHandle<T>, Arg],
      InjectedScriptPoll<R>
    >,
    arg: Arg,
  ): Promise<R | 'error:notconnected'> {
    try {
    ...
    await this._page._frameManager.waitForSignalsCreatedBy(
      ...
      async () => {
        return main.evaluate(
          ([injected, node, { type, eventInit }]) => injected.dispatchEvent(node, type, eventInit),
          [await main.injectedScript(), this, { type, eventInit }] as const,
        );
      },
    );
  ...
  private async _clickablePoint(): Promise<
    types.Point | 'error:notvisible' | 'error:notinviewport'
  > {
  ...
  private async _offsetPoint(
    offset: types.Point,
  ): Promise<types.Point | 'error:notvisible' | 'error:notconnected'> {
  ...
  async _retryPointerAction(
    ...
    options: types.PointerActionOptions &
  ...
  > {
    ...
    if (forceScrollOptions) {
      ...
      const scrolled = await this._scrollRectIntoViewIfNeeded(
        position ? { x: position.x, y: position.y, width: 0, height: 0 } : undefined,
      );
    ...
    if (process.env.PLAYWRIGHT_NO_LAYOUT_SHIFT_CHECK)
      ...
      return this._finishPointerActionDetectLayoutShift(
        progress,
        actionName,
        point,
        options,
  ...
  private async _finishPointerAction(
    progress: Progress,
    actionName: ActionName,
    point: types.Point,
    options: types.PointerActionOptions &
      ...
      types.NavigatingActionWaitOptions,
    action: (point: types.Point) => Promise<void>,
  ): Promise<'error:notconnected' | { hitTargetDescription: string } | 'done'> {
    if (!options.force) {
  ...
  private async _finishPointerActionDetectLayoutShift(
    progress: Progress,
    actionName: ActionName,
    point: types.Point,
    options: types.PointerActionOptions &
    ...
    if (!options.force) {
      ...
      if (actionName === 'move and up') {
        ...
        const handle = await this.evaluateHandleInUtility(
          ([injected, node, { actionType, trial }]) =>
            injected.setupHitTargetInterceptor(node, actionType, trial),
          { actionType, trial: !!options.trial } as const,
        );
    ...
    const actionResult = await this._page._frameManager.waitForSignalsCreatedBy(
      ...
      async () => {
        ...
        if (hitTargetInterceptionHandle) {
          const stopHitTargetInterception = hitTargetInterceptionHandle
            .evaluate((h) => h.stop())
            .catch((e) => 'done' as const);
  ...
  async selectText(
    metadata: CallMetadata,
    options: types.TimeoutOptions & types.ForceOptions = {},
  ): Promise<void> {
    const controller = new ProgressController(metadata, this);
    return controller.run(async (progress) => {
      progress.throwIfAborted(); // Avoid action that has side-effects.
  ...
  async _setInputFiles(
    ...
    const result = await this.evaluateHandleInUtility(
      ([injected, node, multiple]): Element | undefined => {
        const element = injected.retarget(node, 'follow-label');
        if (!element) return;
  ...
  async check(
    metadata: CallMetadata,
    options: { position?: types.Point } & types.PointerActionWaitOptions &
      types.NavigatingActionWaitOptions,
  ) {
    const controller = new ProgressController(metadata, this);
    return controller.run(async (progress) => {
  ...
  async screenshot(
    metadata: CallMetadata,
    options: ScreenshotOptions & TimeoutOptions = {},
  ): Promise<Buffer> {
    const controller = new ProgressController(metadata, this);
    return controller.run(
  ...
  async querySelectorAll(selector: string): Promise<ElementHandle<Element>[]> {
    ...
    return this._page.selectors._queryAll(
      frame,
      info,
      this._frame === frame ? this : undefined,
      true /* adoptToMain */,
    );
  }
  ...
  async evalOnSelectorAndWaitForSignals(
    ...
    // If we end up in the same frame => use the scope again, line above was noop.
    const handle = pair
      ? await this._page.selectors.query(
          pair.frame,
          pair.info,
          this._frame === pair.frame ? this : undefined,
        )
      : null;
  ...
  async evalOnSelectorAllAndWaitForSignals(
    ...
    const { frame, info } = pair;
    // If we end up in the same frame => use the scope again, line above was noop.
    const arrayHandle = await this._page.selectors._queryArrayInMainWorld(
      frame,
      info,
      this._frame === frame ? this : undefined,
    );
  ...
  async waitForSelector(
    metadata: CallMetadata,
    selector: string,
    options: types.WaitForElementOptions = {},
  ...
  async _waitForDisplayedAtStablePosition(
    progress: Progress,
    force: boolean,
    waitForEnabled: boolean,
  ): Promise<'error:notconnected' | 'done'> {
    if (waitForEnabled) progress.log(`  waiting for element to be visible, enabled and stable`);
...
export function waitForSelectorTask(
  selector: SelectorInfo,
  state: 'attached' | 'detached' | 'visible' | 'hidden',
  omitReturnValue?: boolean,
  root?: ElementHandle,
): SchedulableTask<Element | undefined> {
        ...
        return injected.pollRaf((progress) => {
          ...
          if (lastElement !== element) {
            ...
            if (!element) {
              ...
              progress.log(
                `  selector resolved to ${visible ? 'visible' : 'hidden'} ${injected.previewNode(
                  element,
                )}`,
              );
            }
...
export const kUnableToAdoptErrorMessage =
