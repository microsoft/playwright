...
export class FrameExecutionContext extends js.ExecutionContext {
  ...
  async evaluateExpression(expression: string, options: { isFunction?: boolean, extendedSerialization?: boolean }, arg?: any): Promise<any> {
    return js.evaluateExpression(this, expression, { ...options, returnByValue: true }, arg);
  }

  async evaluateExpressionHandle(expression: string, options: { isFunction?: boolean, extendedSerialization?: boolean }, arg?: any): Promise<js.JSHandle<any>> {
...
export class ElementHandle<T extends Node = Node> extends js.JSHandle<T> {
  ...
  override async evaluateExpression(progress: Progress, expression: string, options: { isFunction?: boolean, world?: types.World, extendedSerialization?: boolean }, arg: any): Promise<any> {
    return await progress.race(this.internalEvaluateExpression(expression, options, arg));
  }

  override async internalEvaluateExpression(expression: string, options: { isFunction?: boolean, world?: types.World, extendedSerialization?: boolean }, arg: any): Promise<any> {
    const context = options.world ? await this._frame.context(options.world) : this._context;
    return await js.evaluateExpression(context, expression, { isFunction: options.isFunction, extendedSerialization: options.extendedSerialization, returnByValue: true }, this, arg);
  }
