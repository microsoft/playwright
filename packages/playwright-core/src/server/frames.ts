...
export type SchedulableTask<T> = (
  injectedScript: js.JSHandle<InjectedScript>,
) => Promise<js.JSHandle<InjectedScriptPoll<T>>>;
export type DomTaskBody<T, R, E> = (
...
export class Frame extends SdkObject {
  ...
  constructor(page: Page, id: string, parentFrame: Frame | null) {
    ...
    this._contextData.set('main', {
      contextPromise: new ManualPromise(),
      context: null,
  ...
  async nonStallingEvaluateInExistingContext(
    expression: string,
  ...
  async goto(
    ...
    url: string,
    options: types.GotoOptions = {},
  ): Promise<network.Response | null> {
    const constructedNavigationURL = constructURLBasedOnBaseURL(
  ...
  private async _goto(
    ...
    return this.raceNavigationAction(async () => {
      ...
      const sameDocument = helper.waitForEvent(
        progress,
        this,
        Frame.Events.Navigation,
        (e: NavigationEvent) => !e.newDocument,
      );
      ...
      if (navigateResult.newDocumentId) {
        ...
        event = await helper.waitForEvent(
          progress,
          this,
          Frame.Events.Navigation,
      ...
      if (!this._subtreeLifecycleEvents.has(waitUntil))
        await helper.waitForEvent(
          progress,
          this,
          Frame.Events.AddLifecycle,
          (e: types.LifecycleEvent) => e === waitUntil,
        ).promise;
  ...
  async _waitForNavigation(
    ...
    const navigationEvent: NavigationEvent = await helper.waitForEvent(
      progress,
      this,
      Frame.Events.Navigation,
      (event: NavigationEvent) => {
  ...
  async waitForSelector(
    metadata: CallMetadata,
    selector: string,
    options: types.WaitForElementOptions & { omitReturnValue?: boolean },
    ...
    return controller.run(async (progress) => {
      ...
      return this.retryWithProgress(
        ...
        async (selectorInFrame, continuePolling) => {
          ...
          const result = actualScope
            ? await frame._runWaitForSelectorTaskOnce(
                progress,
                stringifySelector(info.parsed),
                info.world,
                task,
              )
  ...
  async dispatchEvent(
    ...
    type: string,
    eventInit: Object = {},
    options: types.QueryOnSelectorOptions = {},
  ): Promise<void> {
  ...
  async addScriptTag(params: {
    ...
    return this._raceWithCSPError(async () => {
      ...
      const result = (
        await context.evaluateHandle(addScriptContent, { content: content!, type })
      ).asElement()!;
  ...
  private async _raceWithCSPError(
    func: () => Promise<dom.ElementHandle>,
  ): Promise<dom.ElementHandle> {
  ...
  async click(
    metadata: CallMetadata,
    selector: string,
    options: types.MouseClickOptions &
    ...
    return controller.run(async (progress) => {
      return dom.assertDone(
        await this._retryWithProgressIfNotConnected(progress, selector, options.strict, (handle) =>
          handle._click(progress, options),
        ),
      );
    }, this._page._timeoutSettings.timeout(options));
  }
  ...
  async dblclick(
    metadata: CallMetadata,
    selector: string,
    options: types.MouseMultiClickOptions &
      types.PointerActionWaitOptions &
      types.NavigatingActionWaitOptions = {},
  ) {
    const controller = new ProgressController(metadata, this);
  ...
  async dragAndDrop(
    ...
    source: string,
    target: string,
    options: types.DragActionOptions &
      types.PointerActionWaitOptions &
      types.NavigatingActionWaitOptions = {},
  ) {
    const controller = new ProgressController(metadata, this);
  ...
  async tap(
    metadata: CallMetadata,
    selector: string,
    options: types.PointerActionWaitOptions & types.NavigatingActionWaitOptions,
  ) {
    const controller = new ProgressController(metadata, this);
    return controller.run(async (progress) => {
      return dom.assertDone(
  ...
  async fill(
    ...
    selector: string,
    value: string,
    options: types.NavigatingActionWaitOptions & { force?: boolean },
  ) {
    const controller = new ProgressController(metadata, this);
    return controller.run(async (progress) => {
      return dom.assertDone(
  ...
  async focus(
    metadata: CallMetadata,
    selector: string,
    options: types.TimeoutOptions & types.StrictOptions = {},
  ) {
    const controller = new ProgressController(metadata, this);
    await controller.run(async (progress) => {
      dom.assertDone(
  ...
  async highlight(selector: string) {
    ...
    const injectedScript = await context.injectedScript();
    return await injectedScript.evaluate(
      (injected, { parsed }) => {
  ...
  async selectOption(
    metadata: CallMetadata,
    selector: string,
    elements: dom.ElementHandle[],
    values: types.SelectOption[],
    options: types.NavigatingActionWaitOptions & types.ForceOptions = {},
  ): Promise<string[]> {
    const controller = new ProgressController(metadata, this);
    return controller.run(async (progress) => {
  ...
  async setInputFiles(
    metadata: CallMetadata,
    selector: string,
    files: channels.ElementHandleSetInputFilesParams['files'],
    options: types.NavigatingActionWaitOptions = {},
  ): Promise<void> {
    const controller = new ProgressController(metadata, this);
    return controller.run(async (progress) => {
      return dom.assertDone(
        await this._retryWithProgressIfNotConnected(progress, selector, options.strict, (handle) =>
          handle._setInputFiles(progress, files, options),
        ),
      );
    }, this._page._timeoutSettings.timeout(options));
  }
  ...
  async type(
    ...
    selector: string,
    text: string,
    options: { delay?: number } & types.NavigatingActionWaitOptions = {},
  ) {
    const controller = new ProgressController(metadata, this);
    return controller.run(async (progress) => {
      return dom.assertDone(
  ...
  async expect(
    ...
    if (!options.isNot && options.expression === 'to.be.hidden') omitAttached = true;
    else if (options.isNot && options.expression === 'to.be.visible') omitAttached = true;
    else if (
      !options.isNot &&
      options.expression === 'to.have.count' &&
      options.expectedNumber === 0
    )
      omitAttached = true;
    else if (
      options.isNot &&
    ...
    return controller
      .run(async (outerProgress) => {
        outerProgress.log(`${metadata.apiName}${timeout ? ` with timeout ${timeout}ms` : ''}`);
        return await this._scheduleRerunnableTaskWithProgress(
          outerProgress,
          selector,
  ...
  async _waitForFunctionExpression<R>(
    metadata: CallMetadata,
    expression: string,
    isFunction: boolean | undefined,
    arg: any,
    options: types.WaitForFunctionOptions,
    world: types.World = 'main',
  ): Promise<js.SmartHandle<R>> {
    const controller = new ProgressController(metadata, this);
    ...
    const task: dom.SchedulableTask<R> = (injectedScript) =>
  ...
  private async _scheduleRerunnableTask<T, R>(
    metadata: CallMetadata,
    selector: string,
    body: DomTaskBody<T, R, Element>,
    taskData: T,
    options: types.TimeoutOptions & types.StrictOptions & { mainWorld?: boolean } = {},
  ): Promise<R> {
    const controller = new ProgressController(metadata, this);
    return controller.run(async (progress) => {
      return await this._scheduleRerunnableTaskWithProgress(
        progress,
        selector,
        body as DomTaskBody<T, R, Element | undefined>,
        taskData,
        options,
      );
    }, this._page._timeoutSettings.timeout(options));
  ...
  private async _scheduleRerunnableTaskWithProgress<T, R>(
    ...
    taskData: T,
    options: types.TimeoutOptions &
      types.StrictOptions & {
        mainWorld?: boolean;
        querySelectorAll?: boolean;
        logScale?: boolean;
        omitAttached?: boolean;
      } = {},
  ): Promise<R> {
    ...
    return this.retryWithProgress(progress, selector, options, async (selectorInFrame) => {
      ...
      const { frame, info } = selectorInFrame || {
        ...
        info: {
          parsed: {
            parts: [{ name: 'control', body: 'return-empty', source: 'control=return-empty' }],
          },
          world: 'utility',
          strict: !!options.strict,
        },
      };
      return await frame._scheduleRerunnableTaskInFrame(
        progress,
        info,
        callbackText,
        taskData,
  ...
  private async _scheduleRerunnableTaskInFrame<T, R>(
    ...
    {
      const rerunnableTask = new RerunnableTask<R>(
        data,
        progress,
        (injectedScript) => {
  ...
  private _scheduleRerunnableHandleTask<T>(
    progress: Progress,
    world: types.World,
    task: dom.SchedulableTask<T>,
  ): Promise<js.SmartHandle<T>> {
  ...
  private async _resolveFrameForSelector(
    progress: Progress,
    selector: string,
      ...
      if (!isIframe)
        throw new Error(
          `Selector "${stringifySelector(
            info.parsed,
          )}" resolved to ${element.preview()}, <iframe> was expected`,
        );
  ...
  async resolveFrameForSelectorNoWait(
    selector: string,
    options: types.StrictOptions & types.TimeoutOptions = {},
    scope?: dom.ElementHandle,
  ): Promise<SelectorInFrame | null> {
    ...
    for (let i = 0; i < frameChunks.length - 1; ++i) {
      const info = this._page.parseSelector(frameChunks[i], options);
      const element: dom.ElementHandle<Element> | null = await this._page.selectors.query(
        frame,
        info,
        i === 0 ? scope : undefined,
      );
  ...
  private async _runWaitForSelectorTaskOnce<T>(
    progress: Progress,
    selector: string,
    world: types.World,
    task: dom.SchedulableTask<T>,
...
class RerunnableTask<T> {
  ...
  constructor(
    ...
    if (returnByValue) this.promise = new ManualPromise<T>();
    else this.handlePromise = new ManualPromise<js.SmartHandle<T>>();
...
class SignalBarrier {
  ...
  async addFrameNavigation(frame: Frame) {
    ...
    if (frame.parentFrame()) return;
    this.retain();
    const waiter = helper.waitForEvent(
      null,
      frame,
      Frame.Events.Navigation,
      (e: NavigationEvent) => {
