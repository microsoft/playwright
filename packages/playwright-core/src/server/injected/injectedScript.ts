...
export class InjectedScript {
  ...
  private expectArray(elements: Element[], options: FrameExpectParams): { matches: boolean, received?: any } {
    ...
    if (expression === 'to.have.text.array' || expression === 'to.contain.text.array')
      received = elements.map(e => options.useInnerText ? (e as HTMLElement).innerText : elementText(new this.builtins.Map(), e).full);
    ...
    if (received && options.expectedText) {
      ...
      const matchers = options.expectedText.map(e => new ExpectedTextMatcher(this.builtins, e));
