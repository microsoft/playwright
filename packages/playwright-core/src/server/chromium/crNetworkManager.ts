...
export class CRNetworkManager {
  ...
  _onRequestPaused(sessionInfo: SessionInfo, event: Protocol.Fetch.requestPausedPayload) {
    ...
    if (!event.networkId) {
      this._onRequest(sessionInfo, null, event);
      return;
    }
  ...
  _onRequest(sessionInfo: SessionInfo, requestWillBeSentEvent: Protocol.Network.requestWillBeSentPayload | null, requestPausedEvent: Protocol.Fetch.requestPausedPayload | null) {
    const eventRequest = requestWillBeSentEvent?.request ?? requestPausedEvent?.request;
    const requestId = requestWillBeSentEvent?.requestId ?? requestPausedEvent?.requestId;
    if (!eventRequest || !requestId || eventRequest.url.startsWith('data:'))
      return;
    let redirectedFrom: InterceptableRequest | null = null;
    if (requestWillBeSentEvent?.redirectResponse) {
      const request = this._requestIdToRequest.get(requestId);
    ...
    const frameId = requestWillBeSentEvent?.frameId ?? requestPausedEvent?.frameId;
    let frame = frameId ? this._page?._frameManager.frame(frameId) : sessionInfo.workerFrame;
    ...
    if (!frame && this._page && frameId === (this._page?._delegate as CRPage)._targetId) {
      // Main resource request for the page is being intercepted so the Frame is not created
      // yet. Precreate it here for the purposes of request interception. It will be updated
      // later as soon as the request continues and we receive frame tree from the page.
      frame = this._page._frameManager.frameAttached(frameId, null);
    }
    ...
    const isNavigationRequest = requestId === requestWillBeSentEvent?.loaderId && requestWillBeSentEvent?.type === 'Document';
...
class InterceptableRequest {
  ...
  constructor(options: {
    ...
    requestWillBeSentEvent: Protocol.Network.requestWillBeSentPayload | null;
    ...
    this._timestamp = requestWillBeSentEvent?.timestamp ?? monotonicTime();
    // in seconds
    this._wallTime = requestWillBeSentEvent?.wallTime ?? Math.floor(Date.now() / 1000);
    this._requestId = requestWillBeSentEvent?.requestId ?? requestPausedEvent?.requestId!;
    ...
    } = requestPausedEvent?.request ?? requestWillBeSentEvent?.request!;
    const type = (requestPausedEvent?.resourceType ?? requestWillBeSentEvent?.type ?? '').toLowerCase();
