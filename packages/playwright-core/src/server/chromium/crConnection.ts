...
export class CRConnection extends EventEmitter {
  ...
  _onClose() {
    ...
    const browserDisconnectedLogs = helper.formatBrowserLogs(
...
export class CRSession extends EventEmitter {
  ...
  async send<T extends keyof Protocol.CommandParameters>(
    ...
    return new Promise((resolve, reject) => {
      this._callbacks.set(id, {
        resolve,
        reject,
        error: new ProtocolError(false),
        method,
      });
    });
  }

  _sendMayFail<T extends keyof Protocol.CommandParameters>(
    method: T,
    params?: Protocol.CommandParameters[T]
  ): Promise<Protocol.CommandReturnValues[T] | void> {
    return this.send(method, params).catch((error: ProtocolError) =>
  ...
  _onClosed(browserDisconnectedLogs: string | undefined) {
    console.log("# crConnection closed");
