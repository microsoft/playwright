...
export class CRBrowser extends Browser {
  ...
  static async connect(
    ...
    console.log("# before __testHookOnConnectToBrowser");
    if ((options as any).__testHookOnConnectToBrowser)
      await (options as any).__testHookOnConnectToBrowser();

    console.log("# before Browser.getVersion");
    const version = await session.send("Browser.getVersion");
    browser._version = version.product.substring(
      version.product.indexOf("/") + 1
    );
    console.log("# after Browser.getVersion");
    ...
    if (!options.persistent) {
      console.log("# before Target.setAutoAttach");
    ...
    console.log("# before auto attach");
    await Promise.all([
      session
    ...
    console.log("# _waitForAllPagesToBeInitialized");
  ...
  async doCreateNewContext(
    ...
    if (options.proxy) {
      if (process.env.PLAYWRIGHT_DISABLE_FORCED_CHROMIUM_PROXIED_LOOPBACK)
        ...
        proxyBypassList =
          "<-loopback>" +
          (options.proxy.bypass ? `,${options.proxy.bypass}` : "");
    }
  ...
  _onAttachedToTarget({
    ...
    sessionId,
    waitingForDebugger,
  }: Protocol.Target.attachedToTargetPayload) {
    ...
    if (targetInfo.type === "background_page") {
      const backgroundPage = new CRPage(
        session,
        targetInfo.targetId,
        context,
        null,
  ...
  async startTracing(
    ...
    this._tracingClient = page
      ? (page._delegate as CRPage)._mainFrameSession._client
      : this._session;
  ...
  async stopTracing(): Promise<Buffer> {
    ...
    const result = await readProtocolStream(
      this._tracingClient,
      (event as any).stream!,
      this._tracingPath
    );
  ...
  constructor(
    ...
    session.once("Runtime.executionContextCreated", (event) => {
      this._createExecutionContext(
        new CRExecutionContext(session, event.context)
      );
    });
...
export class CRBrowserContext extends BrowserContext {
  ...
  override async _initialize() {
    ...
    console.info(`# cr browser initializing`);
    await Promise.all(promises);
    console.info(`# cr browser initialized`);
  }
