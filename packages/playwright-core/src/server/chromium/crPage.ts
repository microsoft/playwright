...
export class CRPage implements PageDelegate {
    ...
    await Promise.all(
      frameSessions.map((frameSession) => {
        ...
        return cb(frameSession).catch((e) => {
          ...
          if (
            e.message &&
            (e.message.includes('Target closed.') || e.message.includes('Session closed.'))
          )
            return;
          throw e;
        });
  ...
  async exposeBinding(binding: PageBinding) {
    ...
    await Promise.all(
      this._page
        .frames()
        .map((frame) => frame.evaluateExpression(binding.source, false, {}).catch((e) => {})),
    );
  }
  ...
  async takeScreenshot(
    ...
    fitsViewport: boolean | undefined,
  ): Promise<Buffer> {
...
class FrameSession {
  ...
  constructor(
    ...
    client: CRSession,
    targetId: string,
    parentSession: FrameSession | null,
  ) {
    this._client = client;
    this._crPage = crPage;
    this._page = crPage._page;
    this._targetId = targetId;
    this._networkManager = new CRNetworkManager(
      client,
      this._page,
  ...
  private _addRendererListeners() {
    this._eventListeners.push(
      ...[
        eventsHelper.addEventListener(this._client, 'Log.entryAdded', (event) =>
          this._onLogEntryAdded(event),
        ),
        eventsHelper.addEventListener(this._client, 'Page.fileChooserOpened', (event) =>
          this._onFileChooserOpened(event),
        ),
        eventsHelper.addEventListener(this._client, 'Page.frameAttached', (event) =>
          this._onFrameAttached(event.frameId, event.parentFrameId),
        ),
        eventsHelper.addEventListener(this._client, 'Page.frameDetached', (event) =>
          this._onFrameDetached(event.frameId, event.reason),
        ),
        eventsHelper.addEventListener(this._client, 'Page.frameNavigated', (event) =>
          this._onFrameNavigated(event.frame, false),
        ),
        eventsHelper.addEventListener(this._client, 'Page.frameRequestedNavigation', (event) =>
          this._onFrameRequestedNavigation(event),
        ),
        eventsHelper.addEventListener(this._client, 'Page.frameStoppedLoading', (event) =>
          this._onFrameStoppedLoading(event.frameId),
        ),
        eventsHelper.addEventListener(this._client, 'Page.javascriptDialogOpening', (event) =>
          this._onDialog(event),
        ),
        eventsHelper.addEventListener(this._client, 'Page.navigatedWithinDocument', (event) =>
          this._onFrameNavigatedWithinDocument(event.frameId, event.url),
        ),
        eventsHelper.addEventListener(this._client, 'Runtime.bindingCalled', (event) =>
          this._onBindingCalled(event),
        ),
        eventsHelper.addEventListener(this._client, 'Runtime.consoleAPICalled', (event) =>
          this._onConsoleAPI(event),
        ),
        eventsHelper.addEventListener(this._client, 'Runtime.exceptionThrown', (exception) =>
          this._handleException(exception.exceptionDetails),
        ),
        eventsHelper.addEventListener(this._client, 'Runtime.executionContextCreated', (event) =>
          this._onExecutionContextCreated(event.context),
        ),
        eventsHelper.addEventListener(this._client, 'Runtime.executionContextDestroyed', (event) =>
          this._onExecutionContextDestroyed(event.executionContextId),
        ),
        eventsHelper.addEventListener(this._client, 'Runtime.executionContextsCleared', (event) =>
          this._onExecutionContextsCleared(),
        ),
        eventsHelper.addEventListener(this._client, 'Target.attachedToTarget', (event) =>
          this._onAttachedToTarget(event),
        ),
        eventsHelper.addEventListener(this._client, 'Target.detachedFromTarget', (event) =>
  ...
  private _addBrowserListeners() {
    this._eventListeners.push(
      ...[
        eventsHelper.addEventListener(this._client, 'Inspector.targetCrashed', (event) =>
          this._onTargetCrashed(),
        ),
        eventsHelper.addEventListener(this._client, 'Page.screencastFrame', (event) =>
          this._onScreencastFrame(event),
        ),
        eventsHelper.addEventListener(this._client, 'Page.windowOpen', (event) =>
      ...
      this._client.send('Page.getFrameTree').then(({ frameTree }) => {
        ...
        if (isInitialEmptyPage) {
          ...
          lifecycleEventsEnabled
            ...
            .then(() => {
              this._eventListeners.push(
                eventsHelper.addEventListener(this._client, 'Page.lifecycleEvent', (event) =>
  ...
  _onAttachedToTarget(event: Protocol.Target.attachedToTargetPayload) {
    ...
    session.on('Runtime.exceptionThrown', (exception) =>
      this._page.emit(Page.Events.PageError, exceptionToError(exception.exceptionDetails)),
    );
  ...
  async _createVideoRecorder(
    screencastId: string,
    options: types.PageScreencastOptions,
  ): Promise<void> {
    assert(!this._screencastId);
    const ffmpegPath = registry
      .findExecutable('ffmpeg')!
      .executablePathOrDie(this._page._browserContext._browser.options.sdkLanguage);
  ...
  async _updateExtraHTTPHeaders(initial: boolean): Promise<void> {
    ...
    if (!initial || headers.length)
      await this._client.send('Network.setExtraHTTPHeaders', {
        headers: headersArrayToObject(headers, false /* lowerCase */),
      });
  }
  ...
  async _updateEmulateMedia(initial: boolean): Promise<void> {
    ...
    const reducedMotion =
      this._page._state.reducedMotion === null ? '' : this._page._state.reducedMotion;
    const forcedColors =
      this._page._state.forcedColors === null ? '' : this._page._state.forcedColors;
    const features = [
  ...
  async _scrollRectIntoViewIfNeeded(
    handle: dom.ElementHandle,
    rect?: types.Rect,
  ): Promise<'error:notvisible' | 'error:notconnected' | 'done'> {
  ...
  async _adoptElementHandle<T extends Node>(
    handle: dom.ElementHandle<T>,
    to: dom.FrameExecutionContext,
  ): Promise<dom.ElementHandle<T>> {
    const nodeInfo = await this._client.send('DOM.describeNode', {
    ...
    return this._adoptBackendNodeId(nodeInfo.node.backendNodeId, to) as Promise<
      dom.ElementHandle<T>
    >;
  }
