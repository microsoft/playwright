...
export class Screenshotter {
  ...
  private async _originalViewportSize(
    progress: Progress,
  ): Promise<{ viewportSize: types.Size; originalViewportSize: types.Size | null }> {
  ...
  async screenshotElement(
    ...
    return this._queue.postTask(async () => {
      ...
      const buffer = await this._screenshot(
        progress,
        format,
        helper.enclosingIntRect(documentRect),
        undefined,
        fitsViewport,
        options,
      );
  ...
  async _preparePageForScreenshot(progress: Progress, disableAnimations: boolean) {
    await Promise.all(
      this._page.frames().map(async (frame) => {
        await frame
          .nonStallingEvaluateInExistingContext(
            '(' +
              function (disableAnimations: boolean) {
  ...
  async _restorePageAfterScreenshot() {
    await Promise.all(
      this._page.frames().map(async (frame) => {
        frame
          .nonStallingEvaluateInExistingContext(
            'window.__cleanupScreenshot && window.__cleanupScreenshot()',
            false,
            'utility',
          )
          .catch(() => {});
