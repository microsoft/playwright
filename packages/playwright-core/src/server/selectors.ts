...
export class Selectors {
  ...
  async _queryArrayInMainWorld(
    frame: frames.Frame,
    info: SelectorInfo,
    scope?: dom.ElementHandle,
  ): Promise<js.JSHandle<Element[]>> {
  ...
  async _queryAll(
    ...
    scope?: dom.ElementHandle,
    adoptToMain?: boolean,
  ): Promise<dom.ElementHandle<Element>[]> {
    ...
    for (const property of properties.values()) {
      ...
      if (elementHandle) result.push(this._adoptIfNeeded(elementHandle, targetContext));
      else property.dispose();
    }
    return Promise.all(result);
  }

  private async _adoptIfNeeded<T extends Node>(
    handle: dom.ElementHandle<T>,
    context: dom.FrameExecutionContext,
