...
export function fetchData(
  params: HTTPRequestParams,
  onError?: (params: HTTPRequestParams, response: http.IncomingMessage) => Promise<Error>,
): Promise<string> {
...
export async function download(url: string, destination: string, options: DownloadOptions = {}) {
  ...
  return (downloadedBytes: number, totalBytes: number) => {
    ...
    if (!progressBar) {
      progressBar = new ProgressBar(
        `Downloading ${progressBarName} - ${toMegabytes(totalBytes)} [:bar] :percent :etas `,
        {
          complete: '=',
          incomplete: ' ',
          width: 20,
...
export function objectToArray(map?: { [key: string]: any }): NameValue[] | undefined {
  ...
  const result = [];
  for (const [name, value] of Object.entries(map)) result.push({ name, value: String(value) });
  return result;
}
...
function determineUserAgent(): string {
  ...
  if (process.platform === 'win32') {
    ...
    const version = execSync('sw_vers -productVersion', { stdio: ['ignore', 'pipe', 'ignore'] })
      .toString()
      .trim()
      .split('.');
    osIdentifier = 'macOS';
...
export const hostPlatform = ((): HostPlatform => {
  ...
  if (platform === 'linux') {
    ...
    if (parseInt(ubuntuVersion, 10) <= 19) return ('ubuntu18.04' + archSuffix) as HostPlatform;
...
export async function transformCommandsForRoot(
  commands: string[],
): Promise<{ command: string; args: string[]; elevatedPermissions: boolean }> {
