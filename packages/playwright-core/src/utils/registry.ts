...
export class Registry {
  ...
  constructor(browsersJSON: BrowsersJSON) {
    ...
    const findExecutablePath = (dir: string, name: keyof typeof EXECUTABLE_PATHS) => {
      ...
      if (hostPlatform.startsWith('ubuntu')) tokens = EXECUTABLE_PATHS[name]['linux'];
      else if (hostPlatform.startsWith('mac')) tokens = EXECUTABLE_PATHS[name]['mac'];
      else if (hostPlatform.startsWith('win')) tokens = EXECUTABLE_PATHS[name]['win'];
      return tokens ? path.join(dir, ...tokens) : undefined;
    };
    const executablePathOrDie = (
      name: string,
      e: string | undefined,
      installByDefault: boolean,
      sdkLanguage: string,
    ...
    this._executables.push({
      ...
      validateHostRequirements: (sdkLanguage: string) =>
      ...
      _install: () =>
        this._downloadExecutable(
          chromium,
          chromiumExecutable,
          DOWNLOAD_PATHS['chromium'][hostPlatform],
    ...
    this._executables.push({
      ...
      _install: () =>
        this._downloadExecutable(
          chromiumWithSymbols,
          chromiumWithSymbolsExecutable,
          DOWNLOAD_PATHS['chromium-with-symbols'][hostPlatform],
          'PLAYWRIGHT_CHROMIUM_DOWNLOAD_HOST',
        ),
      _dependencyGroup: 'chromium',
    });
    ...
    this._executables.push({
      ...
      _install: () =>
        this._downloadExecutable(
          firefox,
          firefoxExecutable,
          DOWNLOAD_PATHS['firefox'][hostPlatform],
    ...
    this._executables.push({
      ...
      _install: () =>
        this._downloadExecutable(
          firefoxBeta,
          firefoxBetaExecutable,
          DOWNLOAD_PATHS['firefox-beta'][hostPlatform],
          'PLAYWRIGHT_FIREFOX_DOWNLOAD_HOST',
        ),
      _dependencyGroup: 'firefox',
    });
    ...
    this._executables.push({
      ...
      _install: () =>
        this._downloadExecutable(
          webkit,
          webkitExecutable,
          DOWNLOAD_PATHS['webkit'][hostPlatform],
    ...
    this._executables.push({
      ...
      _install: () =>
        this._downloadExecutable(
          ffmpeg,
          ffmpegExecutable,
          DOWNLOAD_PATHS['ffmpeg'][hostPlatform],
  ...
  private _createChromiumChannel(
    ...
    lookAt: Record<'linux' | 'darwin' | 'win32', string>,
  ...
  private async _validateHostRequirements(
    sdkLanguage: string,
    browserName: BrowserName,
    browserDirectory: string,
    linuxLddDirectories: string[],
    dlOpenLibraries: string[],
    windowsExeAndDllDirectories: string[],
  ) {
    ...
    if (os.platform() === 'linux')
      return await validateDependenciesLinux(
        sdkLanguage,
        linuxLddDirectories.map((d) => path.join(browserDirectory, d)),
        dlOpenLibraries,
      );
    if (os.platform() === 'win32' && os.arch() === 'x64')
      return await validateDependenciesWindows(
        windowsExeAndDllDirectories.map((d) => path.join(browserDirectory, d)),
      );
  }
  ...
  async install(executablesToInstall: Executable[]) {
    ...
    try {
      ...
      for (const executable of executables) {
        ...
        else throw new Error(`ERROR: Playwright does not support installing ${executable.name}`);
      }
    } catch (e) {
      if (e.code === 'ELOCKED') {
  ...
  private async _downloadExecutable(
    ...
    downloadPathTemplate: string | undefined,
    downloadHostEnv: string,
  ) {
  ...
  private async _installMSEdgeChannel(
    channel: 'msedge' | 'msedge-beta' | 'msedge-dev',
    scripts: Record<'linux' | 'darwin' | 'win32', string>,
  ) {
    const scriptArgs: string[] = [];
    if (process.platform !== 'linux') {
      const products = JSON.parse(
        await fetchData({ url: 'https://edgeupdates.microsoft.com/api/products' }),
      );
      const productName = {
        'msedge': 'Stable',
        'msedge-beta': 'Beta',
      ...
      const artifact = release
        ? release.Artifacts.find((artifact: any) => artifact.ArtifactName === searchConfig.artifact)
        : null;
  ...
  private async _installChromiumChannel(
    channel: string,
    scripts: Record<'linux' | 'darwin' | 'win32', string>,
    scriptArgs: string[] = [],
  ) {
    ...
    if (isPowerShell) {
      ...
      const { command, args, elevatedPermissions } = await transformCommandsForRoot([
        `bash "${path.join(BIN_PATH, scriptName)}" ${scriptArgs.join('')}`,
      ]);
