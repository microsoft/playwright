...
export class BrowserContextDispatcher
  extends Dispatcher<BrowserContext, channels.BrowserContextChannel>
  implements channels.BrowserContextChannel
{
  ...
  constructor(scope: DispatcherScope, context: BrowserContext) {
    ...
    if (context._browser.options.name === 'chromium') {
      ...
      for (const serviceWorker of (context as CRBrowserContext).serviceWorkers())
        this._dispatchEvent('serviceWorker', {
          worker: new WorkerDispatcher(this._scope, serviceWorker),
        });
      context.on(CRBrowserContext.CREvents.ServiceWorker, (serviceWorker) =>
        this._dispatchEvent('serviceWorker', {
          worker: new WorkerDispatcher(this._scope, serviceWorker),
        }),
      );
    ...
    context.on(BrowserContext.Events.RequestFailed, (request: Request) =>
    ...
    context.on(
      ...
      ({ request, response }: { request: Request; response: Response | null }) =>
  ...
  async setDefaultNavigationTimeoutNoReply(
    params: channels.BrowserContextSetDefaultNavigationTimeoutNoReplyParams,
  ) {
  ...
  async newPage(
    params: channels.BrowserContextNewPageParams,
    metadata: CallMetadata,
  ): Promise<channels.BrowserContextNewPageResult> {
    return { page: lookupDispatcher<PageDispatcher>(await this._context.newPage(metadata)) };
  }

  async cookies(
    params: channels.BrowserContextCookiesParams,
  ): Promise<channels.BrowserContextCookiesResult> {
  ...
  async setExtraHTTPHeaders(
    params: channels.BrowserContextSetExtraHTTPHeadersParams,
  ): Promise<void> {
  ...
  async setNetworkInterceptionEnabled(
    params: channels.BrowserContextSetNetworkInterceptionEnabledParams,
  ): Promise<void> {
    if (!params.enabled) {
  ...
  async storageState(
    params: channels.BrowserContextStorageStateParams,
    metadata: CallMetadata,
  ): Promise<channels.BrowserContextStorageStateResult> {
  ...
  async newCDPSession(
    params: channels.BrowserContextNewCDPSessionParams,
  ): Promise<channels.BrowserContextNewCDPSessionResult> {
    ...
    return {
      session: new CDPSessionDispatcher(
        this._scope,
        await crBrowserContext.newCDPSession(
          (params.page ? (params.page as PageDispatcher) : (params.frame as FrameDispatcher))
            ._object,
        ),
      ),
    };
  ...
  async harExport(
    params: channels.BrowserContextHarExportParams,
  ): Promise<channels.BrowserContextHarExportResult> {
