...
import { toggleTheme, currentTheme } from "@web/theme";
...
export const WorkbenchLoader: React.FunctionComponent<{}> = () => {
  ...
  const [processingErrorMessage, setProcessingErrorMessage] = React.useState<
    string | null
  >(null);
  const [fileForLocalModeError, setFileForLocalModeError] = React.useState<
    string | null
  >(null);
    ...
    (async () => {
      if (traceURLs.length) {
        const swListener = (event: any) => {
          if (event.data.method === "progress") setProgress(event.data.params);
        };
      ...
      {fileForLocalModeError && (
        <div className="drop-target">
          ...
          <div style={{ paddingTop: 20 }}>
            <div>
              1. Click <a href={fileForLocalModeError}>here</a> to put your
              trace into the download shelf
            </div>
            <div>
              2. Go to{" "}
              <a href="https://trace.playwright.dev">trace.playwright.dev</a>
            </div>
