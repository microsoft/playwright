...
function createTree(rootSuite: Suite | undefined, loadErrors: TestError[], projectFilters: Map<string, boolean>): GroupItem {
  ...
  const visitSuite = (projectName: string, parentSuite: Suite, parentGroup: GroupItem) => {
    for (const suite of parentSuite.suites) {
      ...
      let group = parentGroup.children.find(item => item.title === title && item.kind === 'group') as GroupItem | undefined;
      if (!group) {
        group = {
          kind: 'group',
          subKind: 'describe',
    ...
    for (const test of parentSuite.tests) {
      ...
      let testCaseItem = parentGroup.children.find(t => t.title === title && t.kind === 'case') as TestCaseItem;
      if (!testCaseItem) {
        testCaseItem = {
          kind: 'case',
          id: parentGroup.id + '\x1e' + title,
