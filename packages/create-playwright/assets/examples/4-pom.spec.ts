...
test.describe('ToDo App', () => {
  ...
  test('should display zero initial items', async () => {
    await expect(todoPage.listItems).toHaveCount(0);
  });

  test('should be able to add new items', async () => {
  ...
  test('should be able to mark items as completed', async () => {
    await todoPage.addItem('Example #1');
  ...
  test('should be able to filter by uncompleted items', async () => {
    await todoPage.addItem('Example #1');
    await todoPage.addItem('Example #2');
  ...
  test('should be able to filter by completed items', async () => {
    await todoPage.addItem('Example #1');
    await todoPage.addItem('Example #2');
    await todoPage.addItem('Example #3');
    await todoPage.listItems.last().locator('.toggle').check();
  ...
  test('should be able to delete completed items', async () => {
    await todoPage.addItem('Example #1');
    await todoPage.listItems.last().locator('.toggle').check();
    await expect(todoPage.listItems).toHaveCount(1);
