...
test('should render test case', async ({ mount }) => {
  const component = await mount(<TestCaseView projectNames={['chromium', 'webkit']} metadata={{}} rootDir='' test={testCase} prev={undefined} next={undefined} run={0}></TestCaseView>);
...
test('should correctly render links in annotations', async ({ mount }) => {
  const component = await mount(<TestCaseView projectNames={['chromium', 'webkit']} metadata={{}} rootDir='' test={annotationLinkRenderingTestCase} prev={undefined} next={undefined} run={0}></TestCaseView>);
...
test('should correctly render links in attachments', async ({ mount }) => {
  const component = await mount(<TestCaseView projectNames={['chromium', 'webkit']} metadata={{}} rootDir='' test={attachmentLinkRenderingTestCase} prev={undefined} next={undefined} run={0}></TestCaseView>);
...
test('should correctly render prev and next', async ({ mount }) => {
  const component = await mount(<TestCaseView projectNames={['chromium', 'webkit']} metadata={{}} rootDir='' test={attachmentLinkRenderingTestCase} prev={testCaseSummary} next={testCaseSummary} run={0}></TestCaseView>);
  await expect(component).toMatchAriaSnapshot(`
    - text: group
    - link "« previous"
    - link "next »"
...
test('total duration is selected run duration', async ({ mount, page }) => {
  const component = await mount(<TestCaseView projectNames={['chromium', 'webkit']} metadata={{}} rootDir='' test={testCaseWithTwoAttempts} prev={undefined} next={undefined} run={0}></TestCaseView>);
  await expect(component).toMatchAriaSnapshot(`
