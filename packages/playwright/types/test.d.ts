...
/**
 * The [FnAssertions](https://playwright.dev/docs/api/class-fnassertions) class provides assertion methods that can be
 * used to make assertions about mock functions created with `expect.fn()`. Mock functions record their calls and
 * results, so that the test can assert on how they were used, for example when handed to concurrent code as
 * callbacks.
 *
 * Assertions over a mock function are asynchronous and retried until they pass or the expect timeout is reached,
 * similarly to the web-first assertions. Await them to get reliable results:
 *
 * ```js
 * import { test, expect } from '@playwright/test';
 *
...
interface FnAssertions {
  ...
  /**
   * Ensures the mock function has been called exactly the expected number of times. Note that the assertion fails right
   * away when the mock function has been called more times than expected, because the call count can only grow.
   *
   * **Usage**
   *
   * ```js
  ...
  /**
   * Ensures the mock function has been called at least once with the specified arguments. Arguments are compared with
   * the same algorithm as
   * [expect(value).toEqual(expected)](https://playwright.dev/docs/api/class-genericassertions#generic-assertions-to-equal),
   * so asymmetric matchers like `expect.objectContaining()` are supported.
   *
   * **Usage**
   *
   * ```js
  ...
  /**
   * Ensures the last call of the mock function resolved with the specified value. The assertion waits for the pending
   * result of the call to settle.
   *
   * **Usage**
   *
   * ```js
  ...
  /**
   * Ensures the last call of the mock function returned the specified value. For mock functions with an async
   * implementation, the returned value is a promise, see
   * [expect(mockFunction).toHaveLastResolvedWith(value)](https://playwright.dev/docs/api/class-fnassertions#fn-assertions-to-have-last-resolved-with)
   * instead.
   *
   * **Usage**
   *
  ...
  /**
   * Ensures the n-th call of the mock function resolved with the specified value. The assertion waits for the pending
   * result of the call to settle.
   *
   * **Usage**
   *
  ...
  /**
   * Ensures the n-th call of the mock function returned the specified value. For mock functions with an async
   * implementation, the returned value is a promise, see
   * [expect(mockFunction).toHaveNthResolvedWith(n, value)](https://playwright.dev/docs/api/class-fnassertions#fn-assertions-to-have-nth-resolved-with)
   * instead.
   *
   * **Usage**
   *
  ...
  /**
   * Ensures the mock function has resolved successfully at least once. A call counts as resolved when its returned
   * promise has been fulfilled, or when it returned a non-promise value.
   *
   * **Usage**
   *
   * ```js
  ...
  /**
   * Ensures the mock function has resolved successfully exactly the expected number of times. Calls that threw,
   * returned a rejected promise, or are still pending do not count.
   *
   * **Usage**
   *
   * ```js
   ...
   * ```
   *
   * @param count Expected number of resolved calls.
   */
  toHaveResolvedTimes(count: number): Promise<void>;
  /**
   * Ensures the mock function has resolved with the specified value at least once. Values are compared with the same
   * algorithm as
   * [expect(value).toEqual(expected)](https://playwright.dev/docs/api/class-genericassertions#generic-assertions-to-equal),
   * so asymmetric matchers like `expect.objectContaining()` are supported.
   *
  ...
  /**
   * Ensures the mock function has returned at least once, i.e. has been called and did not throw. Note that a mock
   * function with an async implementation returns a promise and therefore counts as returned even if that promise is
   * later rejected.
   *
   * **Usage**
   *
   ...
   * ```
   *
   * @param count Expected number of returns.
   */
  toHaveReturnedTimes(count: number): Promise<void>;
  /**
   * Ensures the mock function has returned the specified value at least once. Values are compared with the same
   * algorithm as
   * [expect(value).toEqual(expected)](https://playwright.dev/docs/api/class-genericassertions#generic-assertions-to-equal),
   * so asymmetric matchers like `expect.objectContaining()` are supported. For mock functions with an async
   * implementation, the returned value is a promise, see
   * [expect(mockFunction).toHaveResolvedWith(value)](https://playwright.dev/docs/api/class-fnassertions#fn-assertions-to-have-resolved-with)
   * instead.
   *
   * **Usage**
   *
