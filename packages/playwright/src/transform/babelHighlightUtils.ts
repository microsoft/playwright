...
function containsPosition(location: T.SourceLocation, position: SourcePosition): boolean {
...
export function findTestEndPosition(text: string, location: Location): Location | undefined {
  ...
  traverse(ast, {
    enter(path) {
      if (t.isCallExpression(path.node) && path.node.loc && containsPosition(path.node.loc, { line: location.line, column: location.column })) {
