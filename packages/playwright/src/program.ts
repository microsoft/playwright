...
function addListFilesCommand(program: Command) {
  ...
  command.option('--project <project-name...>', `Only run tests from matching this regular expression (default: list all projects)`);
  command.action(async (args, opts) => {
    try {
...
const testOptions: [string, string][] = [
  ...
  ['--project <project-name...>', `Only run tests matching this regular expression (default: run all projects)`],
