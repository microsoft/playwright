...
const localStorageList = defineTabTool({
  ...
  handle: async (tab, params, response) => {
    ...
    response.addCode(`// List all localStorage items`);
  },
});
...
const localStorageGet = defineTabTool({
  ...
  handle: async (tab, params, response) => {
    ...
    response.addCode(`localStorage.getItem('${params.key}');`);
  },
});
...
const localStorageSet = defineTabTool({
  ...
  handle: async (tab, params, response) => {
    ...
    response.addTextResult(`localStorage '${params.key}' set successfully`);
    response.addCode(`localStorage.setItem('${params.key}', '${params.value}');`);
  },
});
...
const localStorageDelete = defineTabTool({
  ...
  handle: async (tab, params, response) => {
    ...
    response.addCode(`localStorage.removeItem('${params.key}');`);
  },
});
...
const localStorageClear = defineTabTool({
  ...
  handle: async (tab, params, response) => {
    ...
    response.addCode(`localStorage.clear();`);
  },
});
...
const sessionStorageList = defineTabTool({
  ...
  handle: async (tab, params, response) => {
    ...
    if (items.length === 0)
      ...
      response.addTextResult(items.map(item => `${item.key}=${item.value}`).join('\n'));
    response.addCode(`// List all sessionStorage items`);
  },
});
...
const sessionStorageGet = defineTabTool({
  ...
  handle: async (tab, params, response) => {
    ...
    if (value === null)
      ...
      response.addTextResult(`${params.key}=${value}`);
    response.addCode(`sessionStorage.getItem('${params.key}');`);
  },
});
...
const sessionStorageSet = defineTabTool({
  ...
  handle: async (tab, params, response) => {
    ...
    response.addTextResult(`sessionStorage '${params.key}' set successfully`);
    response.addCode(`sessionStorage.setItem('${params.key}', '${params.value}');`);
  },
});
...
const sessionStorageDelete = defineTabTool({
  ...
  handle: async (tab, params, response) => {
    ...
    response.addCode(`sessionStorage.removeItem('${params.key}');`);
  },
});
...
const sessionStorageClear = defineTabTool({
  ...
  handle: async (tab, params, response) => {
    ...
    response.addCode(`sessionStorage.clear();`);
  },
});
