...
`);
  ...
  async onExit() {
    ...
    const shouldOpen = this._options.tty === 'interactive' && (this._open === 'always' || (!ok && this._open === 'on-failure'));
    if (shouldOpen) {
      await showHTMLReport(this._outputFolder, this._host, this._port, singleTestId);
    } else if (this._options._mode === 'test' && this._options.tty === 'interactive') {
