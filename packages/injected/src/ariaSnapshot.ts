...
export function generateAriaTree(rootElement: Element, generation: number, includeIframe: boolean): AriaSnapshot {
  ...
  const visit = (ariaNode: AriaNode, node: Node) => {
    ...
    addElement(element);
    const childAriaNode = toAriaNode(element, includeIframe);
    if (childAriaNode)
...
function toAriaNode(element: Element, includeIframe: boolean): AriaNode | null {
...
export function matchesAriaTree(rootElement: Element, template: AriaTemplateNode): { matches: AriaNode[], received: MatcherReceived } {
  const snapshot = generateAriaTree(rootElement, 0, false);
...
export function getAllByAria(rootElement: Element, template: AriaTemplateNode): Element[] {
  const root = generateAriaTree(rootElement, 0, false).root;
