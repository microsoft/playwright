...
const {registryDirectory} = require('playwright-core/lib/coreBundle').registry;
...
/**
 * @param {{ findPackages: (library: string) => Promise<string[]>, pickPackage: (library: string, packages: string[]) => string | null }} opts
 */
async function run({ findPackages, pickPackage }) {
  ...
  for (const [library, packages] of ambiguityLibraries) {
    const pkg = packages.find(p => allUsedPackages.has(p));
    if (!pkg)
      continue;
    libraryToPackage.set(library, pkg);
    ambiguityLibraries.delete(library);
  ...
  for (const [library, packages] of ambiguityLibraries) {
    const question = [
      ...
      ...packages.map((p, index) => `  (${index + 1}) ${p}`),
      'Enter number: ',
    ].join('\n');
  ...
  for (const descriptor of browserDescriptors) {
    for (const library of descriptor.missingLibraries) {
      const pkg = libraryToPackage.get(library);
      if (pkg)
        descriptor.requiredPackages.add(pkg);
      else
  ...
  for (const [library, pkg] of sortedEntries)
    console.log(`  "${library}": "${pkg}",`);
  console.log('}');
