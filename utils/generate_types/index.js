...
function generateKeys() {
  const keys = require(path.join(PROJECT_DIR, 'lib', 'usKeyboardLayout.js')).keyDefinitions;
  const exported = [];
  const nonExported = new Set();
  ...
  for (const key of nonExported) {
    const subsumed = exported.some(other => {
      const v1 = keys[key];
      const v2 = keys[other];
      let matches = true;
      for (const prop in v1)
        matches = matches && v1[prop] === v2[prop];
      return matches;
    });
    if (!subsumed)
      throw new Error(`Key "${key}" does not have an easily printable alternative.`);
  }
  exported.sort((a, b) => {
