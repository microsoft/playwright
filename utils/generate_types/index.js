...
class TypesGenerator {
  ...
  async generateTypes(overridesFile) {
    ...
    const overrides = await parseOverrides(overridesFile, className => {
      ...
      return this.classBody(this.docClassForName(className));
    });

    const classes = this.documentation.classesArray.filter(cls => !handledClasses.has(cls.name));
    return `// This file is generated by ${__filename.substring(path.join(__dirname, '..', '..').length).split(path.sep).join(path.posix.sep)}
${overrides}

${classes.map(classDesc => this.classToString(classDesc)).join('\n')}
