...
function inlineType(type, indent, wrapEnums = false, preserveSerialized = false) {
  if (typeof type === 'string') {
    ...
    if (type === 'SerializedValue')
      return { ts: preserveSerialized ? 'SerializedValue' : 'UnserializedValue', scheme: 'tSerializedValue', optional };
    if (type === 'SerializedArgument')
      return { ts: preserveSerialized ? 'SerializedArgument' : 'UnserializedArgument', scheme: 'tSerializedArgument', optional };
  ...
  if (type.type.startsWith('array')) {
    ...
    const inner = inlineType(type.items, indent, true, preserveSerialized);
  ...
  if (type.type.startsWith('object')) {
    const optional = type.type.endsWith('?');
    const inner = properties(type.properties, indent + '  ', false, preserveSerialized);
    return {
      ts: `{\n${inner.ts}\n${indent}}`,
      scheme: `tObject({\n${inner.scheme}\n${indent}})`,
      optional
...
function properties(properties, indent, onlyOptional, preserveSerialized = false) {
  const ts = [];
  const scheme = [];
  const visitProperties = props => {
    for (const [name, value] of Object.entries(props)) {
      ...
      const inner = inlineType(value, indent, false, preserveSerialized);
...
function objectType(props, indent, onlyOptional = false, preserveSerialized = false) {
  if (!Object.entries(props).length)
    return { ts: `{}`, scheme: `tObject({})` };
  const inner = properties(props, indent + '  ', onlyOptional, preserveSerialized);
...
import { scheme, tOptional, tObject, tBoolean, tInt, tFloat, tString, tAny, tEnum, tArray, tBinary, tChannel, tType, tSerializedValue, tSerializedArgument } from './validatorPrimitives';
...
for (const [name, item] of Object.entries(protocol)) {
  if (item.type === 'interface') {
    ...
    if (name === 'SerializedValue' || name === 'SerializedArgument') {
      // The structural TS type stays as documentation of the wire format, but elsewhere
      // these types are referenced via UnserializedValue / UnserializedArgument (= any),
      // because tSerializedValue / tSerializedArgument validators produce raw JS values
      // at the dispatcher / client boundary.
      const inner = objectType(item.properties, '', false, true);
