...
NODE_VERSION="22.0.0-nightly202402190550bc149c" # autogenerated via ./update-playwright-driver-version.mjs
...
function build {
  ...
  ARCHIVE=$3
  RUN_DRIVER=$4
  if [[ $NODE_VERSION =~ "nightly" ]]; then
    NODE_URL=https://nodejs.org/download/nightly/v${NODE_VERSION}/${NODE_DIR}.${ARCHIVE}
  else
  ...
  DRIVER_DIR="$SCRIPT_PATH/output/playwright-${SUFFIX}"
  npx esbuild --bundle package/cli.js --outdir=package-bundled --platform=node --packages=external
  node ../../generate-sea-config.mjs $DRIVER_DIR
  ./node --experimental-sea-config $DRIVER_DIR/package-bundled/sea-config.json 
  codesign --remove-signature node
  npx -y postject node NODE_SEA_BLOB $DRIVER_DIR/package-bundled/playwright.blob --sentinel-fuse NODE_SEA_FUSE_fce680ab2cc467b6e072b8b5df1996b2 --macho-segment-name NODE_SEA
  codesign --sign - node
