...
async function validateCache(packagePath: string, browsersPath: string, linksDir: string, browserNames?: browserPaths.BrowserName[]) {
  ...
  let myBrowsersToDownload = await readBrowsersToDownload(packagePath, browserNames);
  const browserListFromENV = getBrowserListFromENV();
  if (browserListFromENV) {
    browserFetcher.logPolitely(
      `Downloading browsers ${browserListFromENV.join(
        ", "
      )} as specified in PLAYWRIGHT_BROWSERS_LIST env var.`
    );
    myBrowsersToDownload = myBrowsersToDownload.filter((browser) =>
      browserListFromENV.includes(browser.name)
    );
  }
