...
export class DispatcherConnection {
  private _scopes = new Map<string, DispatcherScope>();
  onmessage = (message: string) => {};

  createScope(guid: string = ''): DispatcherScope {
    const scope = new DispatcherScope(this, guid);
  ...
  async dispatch(message: string) {
    ...
    const { id, scope } = parsedMessage;
    const dispatcherScope = this._scopes.get(scope || '')!;
    if (!dispatcherScope) {
      this.onmessage(JSON.stringify({ id, error: serializeError(new Error('Object is closed')) }));
      return;
    }
    dispatcherScope._dispatch(parsedMessage);
  }
}
...
export class DispatcherScope {
  ...
  constructor(connection: DispatcherConnection, guid?: string) {
    this.connection = connection;
    this._guid = guid;
  }

  async sendMessageToClient(guid: string, method: string, params: any): Promise<any> {
    const message: any = { guid, method, params: this._replaceDispatchersWithGuids(params) };
    if (this._guid)
      message.scope = this._guid;
    this.connection.onmessage(JSON.stringify(message));
  }

  async _dispatch(parsedMessage: any) {
    ...
    try {
      ...
      this.connection.onmessage(JSON.stringify({ id, scope: this._guid, result: this._replaceDispatchersWithGuids(result) }));
    } catch (e) {
      this.connection.onmessage(JSON.stringify({ id, scope: this._guid, error: serializeError(e) }));
    }
  }
...
export class Dispatcher<Type, Initializer> extends EventEmitter implements Channel {
  ...
  constructor(scope: DispatcherScope, object: Type, type: string, initializer: Initializer, createScope = false, guid = type + '@' + helper.guid()) {
    super();
    this._type = type;
    this._guid = guid;
    this._object = object;
    this._scope = createScope ? scope.connection.createScope(guid) : scope;
    scope.dispatchers.set(this._guid, this);
    (object as any)[dispatcherSymbol] = this;
    scope.sendMessageToClient(this._guid, '__create__', { type, initializer });
  }
