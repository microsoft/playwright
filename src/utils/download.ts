...
import { getErrorMessage, httpRequest } from './utils';
...
function downloadFile(
  ...
  const promise: Promise<{ error: any }> = new Promise(x => {
    fulfill = x;
  });
...
export async function download(
  ...
  const { progressBarName = 'file', retryCount = 3 } = options;
  for (let attempt = 1; attempt <= retryCount; ++attempt) {
    ...
    if (!error) {
      debugLogger.log('download', `SUCCESS downloading ${progressBarName}`);
      break;
    }
    const errorMessage = getErrorMessage(error);
    debugLogger.log('download', `attempt #${attempt} - ERROR: ${errorMessage}`);
    if (
      attempt < retryCount &&
      ...
      debugLogger.log('download', `sleeping ${millis}ms before retry...`);
