...
export class ImbaLanguageGenerator implements LanguageGenerator {
  id: string;
  fileName: string;
  highlighter = 'python'; // TODO: use imba
  ...
  generateAction(actionInContext: ActionInContext): string {
    ...
    const formatter = new ImbaFormatter(4);
    formatter.newLine();
    ...
    if (signals.dialog) {
      formatter.add(`  ${pageAlias}.once 'dialog', do(dialog)
    ...
    if (signals.waitForNavigation){
      formatter.add(`${pageAlias}.waitForNavigation(
      #    { url: ${quote(signals.waitForNavigation.url)} }
      ),`);
    }
    // Download signals.
    if (signals.download)
  ...
  generateTestHeader(options: LanguageGeneratorOptions): string {
    const formatter = new ImbaFormatter();
...
${useText ? '\ntest.use(' + useText + ');\n' : ''}
  ...
  generateStandaloneHeader(options: LanguageGeneratorOptions): string {
    const formatter = new ImbaFormatter();
    formatter.add(`const { ${options.browserName}${options.deviceName ? ', devices' : ''} } = require('playwright')`);
    formatter.add(`(do`);
    const formatter2 = new ImbaFormatter(4);
    formatter2.add(`const browser = await ${options.browserName}.launch ${formatObjectOrVoid(options.launchOptions)}`);
    formatter2.add(`const context = await browser.newContext(${formatContextOptions(options.contextOptions, options.deviceName)})`);
    return formatter.format() + '\n' + formatter2.format();
  }
...
export class ImbaFormatter {
  ...
  format(): string {
    ...
    return this._lines.map((line: string) => {
      ...
      if (line.endsWith('{') || line.endsWith('[') || /do\(([^)]+)\)/.test(line) || line.trim().endsWith('do'))
