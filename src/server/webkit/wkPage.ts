...
export class WKPage implements PageDelegate {
  ...
  private async _initializePageProxySession() {
    ...
    if (this._browserContext._options.recordVideos) {
      const size = this._browserContext._options.recordVideos.size || this._browserContext._options.viewport || { width: 1280, height: 720 };
      const outputFile = path.join(this._browserContext._options.recordVideos.path, createGuid() + '.webm');
