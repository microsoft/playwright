...
test('should generate multiple missing', async ({ runInlineTest }, testInfo) => {
  ...
  expect(result.output).toContain(`A snapshot doesn't exist at __snapshots__/a.spec.ts/test-1.yml, writing actual`);
  expect(result.output).toContain(`A snapshot doesn't exist at __snapshots__/a.spec.ts/test-2.yml, writing actual`);
...
test('should rebaseline all', async ({ runInlineTest }, testInfo) => {
  ...
  expect(result.exitCode).toBe(0);
  expect(result.output).toContain(`A snapshot is generated at __snapshots__/a.spec.ts/test-1.yml`);
  expect(result.output).toContain(`A snapshot is generated at __snapshots__/a.spec.ts/test-2.yml`);
  const snapshot1 = await fs.promises.readFile(testInfo.outputPath('__snapshots__/a.spec.ts/test-1.yml'), 'utf8');
  expect(snapshot1).toBe('- heading "hello world" [level=1]');
  const snapshot2 = await fs.promises.readFile(testInfo.outputPath('__snapshots__/a.spec.ts/test-2.yml'), 'utf8');
  expect(snapshot2).toBe('- heading "hello world 2" [level=1]');
...
test('should generate snapshot name', async ({ runInlineTest }, testInfo) => {
  ...
  expect(result.exitCode).toBe(1);
  expect(result.output).toContain(`A snapshot doesn't exist at __snapshots__/a.spec.ts/test-name-1.yml, writing actual`);
  expect(result.output).toContain(`A snapshot doesn't exist at __snapshots__/a.spec.ts/test-name-2.yml, writing actual`);
