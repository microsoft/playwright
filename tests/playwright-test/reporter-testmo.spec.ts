...
for (const useIntermediateMergeReport of [false, true] as const) {
  test.describe(`${useIntermediateMergeReport ? 'merged' : 'created'}`, () => {
    ...
    test('should render unexpected', async ({ runInlineTest }) => {
      ...
      const failure =
        xml['testsuites']['testsuite'][0]['testcase'][0]['failure'][0];
    ...
    test('should render stdout', async ({ runInlineTest }) => {
      ...
      expect(testcase['system-out'][0]).toContain(
          '[33mHello world[39m\nHello again'
      );
      ...
      expect(testcase['failure'][0]['_']).toContain(
          `>  9 |             test.expect("abc").toBe('abcd');`
      );
      expect(result.exitCode).toBe(1);
    });

    test('should render stdout without ansi escapes', async ({
      runInlineTest,
    }) => {
      const result = await runInlineTest(
          {
          ...
          module.exports = {
            reporter: [ ['testmo', { stripANSIControlSequences: true }] ],
          };
        `,
            'a.test.ts': `
          import colors from 'colors/safe';
    ...
    test('should render, by default, character data as CDATA sections', async ({
      runInlineTest,
    }) => {
      const result = await runInlineTest(
          {
    ...
    test('should not render projects if they dont exist', async ({
      runInlineTest,
    }) => {
      const result = await runInlineTest(
          {
      ...
      expect(xml['testsuites']['testsuite'][0]['$']['skipped']).toBe('0');
      expect(
          xml['testsuites']['testsuite'][0]['testcase'][0]['$']['name']
      ).toBe('one');
      expect(
          xml['testsuites']['testsuite'][0]['testcase'][0]['$']['classname']
      ).toBe('a.test.js');
      expect(result.exitCode).toBe(0);
    });
    ...
    test('should render projects', async ({ runInlineTest }) => {
      ...
      expect(xml['testsuites']['testsuite'][0]['$']['name']).toBe('a.test.js');
      expect(xml['testsuites']['testsuite'][0]['$']['hostname']).toBe(
          'project1'
      );
      expect(xml['testsuites']['testsuite'][0]['$']['tests']).toBe('1');
      expect(xml['testsuites']['testsuite'][0]['$']['failures']).toBe('0');
      ...
      expect(xml['testsuites']['testsuite'][1]['$']['hostname']).toBe(
          'project2'
      );
      ...
      expect(
          xml['testsuites']['testsuite'][1]['testcase'][0]['$']['name']
      ).toBe('one');
      expect(
          xml['testsuites']['testsuite'][1]['testcase'][0]['$']['classname']
      ).toBe('a.test.js');
      expect(result.exitCode).toBe(0);
    });

    test('should render existing attachments, but not missing ones', async ({
      runInlineTest,
    }) => {
      test.skip(
          useIntermediateMergeReport,
          'Blob report hashes attachment paths'
      );
      const result = await runInlineTest(
          {
            'a.test.js': `
    ...
    test('should render annotations to custom testcase properties', async ({
      runInlineTest,
    }) => {
      const result = await runInlineTest(
          {
            'playwright.config.ts': `module.exports = { reporter: 'testmo' };`,
            'a.test.js': `
          import { test, expect } from '@playwright/test';
          test('one', async ({}, testInfo) => {
      ...
      expect(testcase['properties'][0]['property'][0]['$']['name']).toBe(
          'test_description'
      );
    ...
    test('should render built-in annotations to testcase properties', async ({
      runInlineTest,
    }) => {
      const result = await runInlineTest(
          {
    ...
    test('should render all annotations to testcase value based properties, if requested', async ({
      runInlineTest,
    }) => {
      const result = await runInlineTest(
          {
        ...
        `,
          ...
          test('one', async ({}, testInfo) => {
            ...
            testInfo.annotations.push({ type: 'html:header', description: '<h1>This is a test!</h1>' });
          });
        `,
          },
          { reporter: '' }
      ...
      expect(testcase['properties'][0]['property'][1]['$']['name']).toBe(
          'test_key'
      );
      expect(testcase['properties'][0]['property'][1]['$']['value']).toBe(
          'CALC-2'
      );
      expect(testcase['properties'][0]['property'][2]['$']['name']).toBe(
          'test_summary'
      );
      expect(testcase['properties'][0]['property'][2]['$']['value']).toBe(
          'sample summary'
      );
      expect(testcase['properties'][0]['property'][3]['$']['name']).toBe(
          'requirements'
      );
      expect(testcase['properties'][0]['property'][3]['$']['value']).toBe(
          'CALC-5,CALC-6'
      );
      expect(testcase['properties'][0]['property'][4]['$']['name']).toBe(
          'html:header'
      );
    ...
    test('should render all steps to properties', async ({
      runInlineTest,
    }) => {
      const result = await runInlineTest(
          {
    ...
    test('should not embed attachments to a custom testcase property, if not explicitly requested', async ({
      runInlineTest,
    }) => {
      const result = await runInlineTest(
          {
    ...
    test.describe('report location', () => {
      test('with config should create report relative to config', async ({
        runInlineTest,
      }, testInfo) => {
        ...
        const result = await runInlineTest(
            ...
            module.exports = { reporter: [['testmo', { outputFile: '../my-report/a.xml' }]] };
          `,
        ...
        expect(
            fs.existsSync(
                testInfo.outputPath(path.join('nested', 'my-report', 'a.xml'))
            )
        ).toBeTruthy();
      });

      test('with env var should create relative to cwd', async ({
        runInlineTest,
      }, testInfo) => {
        const result = await runInlineTest(
            {
          ...
          `,
            ...
            { reporter: 'testmo,line' },
            { PLAYWRIGHT_TESTMO_OUTPUT_NAME: '../my-report.xml' },
            {
        ...
        expect(
            fs.existsSync(
                testInfo.outputPath('foo', 'bar', 'baz', 'my-report.xml')
            )
        ).toBe(true);
      });
    });
