...
const assets = path.join(__dirname, 'assets', 'selenium-grid');
export const standalone314159 = path.join(assets, 'standalone-3.141.59.jar');
export const selenium400rc1 = path.join(assets, 'selenium-4.0.0-rc-1.jar');
export const seleniumConfigStandalone = path.join(__dirname, 'assets', 'selenium-config-standalone.json');
export const chromeDriver = path.join(assets, process.platform === 'win32' ? 'chromedriver.exe' : 'chromedriver');
const chromeDriverZip = path.join(assets, 'chromedriver.zip');
