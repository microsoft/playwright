...
test('should respect custom shard weights', async ({ runInlineTest }) => {
  await test.step('shard 1', async () => {
    const result = await runInlineTest(tests, { 'shard': '1/2', 'workers': 1 }, { PLAYWRIGHT_SHARD_WEIGHTS: '40:60' });
  ...
  await test.step('shard 2', async () => {
    const result = await runInlineTest(tests, { 'shard': '2/2', 'workers': 1 }, { PLAYWRIGHT_SHARD_WEIGHTS: '40:60' });
    expect.soft(result.exitCode).toBe(0);
    expect.soft(result.outputLines).toEqual([
      'a2-test1-done',
      'a2-test2-done',
