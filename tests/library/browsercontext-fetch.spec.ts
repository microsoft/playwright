...
it('should support multipart/form-data and keep the order', async function ({ context, page, asset, server }) {
  // For reference, the graphql-uploads spec: https://github.com/jaydenseric/graphql-multipart-request-spec
  const given = Array(5).fill(undefined)
      .reduce(
          (acc, _, i) => ({ ...acc, [`${i}`]: { name: `f${i}.js`, buffer: Buffer.from(`var x = ${i};\n;console.log(x);`) } }), // Uploaded files in case of graphql-uploads
          {
            'firstField': 'firstFieldContent', // "operation" in case of graphql-uploads
            'secondField': 'secondFieldContent', // "map" in case of graphql-uploads
          });
  ...
  // Comparing Object.keys(fields) with givenKeys is not reliable because the order of keys is not guaranteed.
  // The ordering on L960 and L980 will be the same, as it's generated by the same node engine.
  // const actualKeys = Object.keys(fields);
  // expect(actualKeys).toEqual(givenKeys);
  expect(Object.keys(fields)).toEqual(['firstField', 'secondField', '0', '1', '2', '3', '4']);
  expect(response.status()).toBe(200);
});
