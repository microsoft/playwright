...
async function setupWS(target: Page | Frame, server: TestServer, binaryType: 'blob' | 'arraybuffer', protocols?: string | string[], relativeURL?: boolean) {
  ...
  const wsUrl = relativeURL ? '/ws' : 'ws://' + new URL(server.EMPTY_PAGE).host + '/ws';
  ...
  test.describe(mock, async () => {
    ...
    test('should work with text message', async ({ page, server }) => {
      ...
      await expect.poll(() => page.evaluate(() => window.log)).toEqual([
        'open',
        `message: data=hello origin=ws://${new URL(server.EMPTY_PAGE).host} lastEventId=`,
      ]);
    ...
    test('should work with binaryType=blob', async ({ page, server }) => {
      ...
      await expect.poll(() => page.evaluate(() => window.log)).toEqual([
        'open',
        `message: data=blob:hi origin=ws://${new URL(server.EMPTY_PAGE).host} lastEventId=`,
      ]);
      const messagePromise = new Promise(f => ws.once('message', data => f(data.toString())));
    ...
    test('should work with binaryType=arraybuffer', async ({ page, server }) => {
      ...
      await expect.poll(() => page.evaluate(() => window.log)).toEqual([
        'open',
        `message: data=arraybuffer:hi origin=ws://${new URL(server.EMPTY_PAGE).host} lastEventId=`,
      ]);
      const messagePromise = new Promise(f => ws.once('message', data => f(data.toString())));
...
test('should pattern match', async ({ page, server }) => {
  ...
  }, { host: new URL(server.EMPTY_PAGE).host });
...
test('should work with server', async ({ page, server }) => {
  ...
  await expect.poll(() => page.evaluate(() => window.log)).toEqual([
    'open',
    `message: data=modified origin=ws://${new URL(server.EMPTY_PAGE).host} lastEventId=`,
    `message: data=pass-server origin=ws://${new URL(server.EMPTY_PAGE).host} lastEventId=`,
  ]);
  ...
  await expect.poll(() => page.evaluate(() => window.log)).toEqual([
    ...
    `message: data=modified origin=ws://${new URL(server.EMPTY_PAGE).host} lastEventId=`,
    `message: data=pass-server origin=ws://${new URL(server.EMPTY_PAGE).host} lastEventId=`,
    `message: data=response origin=ws://${new URL(server.EMPTY_PAGE).host} lastEventId=`,
  ]);
  ...
  await expect.poll(() => page.evaluate(() => window.log)).toEqual([
    ...
    `message: data=pass-server origin=ws://${new URL(server.EMPTY_PAGE).host} lastEventId=`,
    `message: data=response origin=ws://${new URL(server.EMPTY_PAGE).host} lastEventId=`,
    `message: data=another origin=ws://${new URL(server.EMPTY_PAGE).host} lastEventId=`,
  ]);
...
test('should work with no trailing slash', async ({ page, server }) => {
  const log: string[] = [];
  // No trailing slash!
  await page.routeWebSocket('ws://' + new URL(server.EMPTY_PAGE).host, ws => {
    ws.onMessage(message => {
...
test('should work with baseURL', async ({ contextFactory, server }) => {
  const context = await contextFactory({ baseURL: 'http://' + new URL(server.EMPTY_PAGE).host });
  ...
  await expect.poll(() => page.evaluate(() => window.log)).toEqual([
    'open',
    `message: data=echo origin=ws://${new URL(server.EMPTY_PAGE).host} lastEventId=`,
  ]);
});
