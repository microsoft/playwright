...
const it = base.extend<{
  launchPersistentContext: (mode: 'cli' | 'cdp', extensionPath: string, options?: Parameters<BrowserType['launchPersistentContext']>[1]) => Promise<BrowserContext>;
      }>({
        launchPersistentContext: async ({ browserType }, use) => {
          const browsers: BrowserContext[] = [];
          await use(async (mode, extensionPath, options = {}) => {
            if (mode === 'cli') {
...
it.describe('MV2', () => {
  // NOTE: MV2 extensions can't be loaded via CDP mode.
...
for (const mode of ['cli', 'cdp'] as const) {
  it.describe(`MV3 (load extension via ${mode})`, () => {
    it.skip(({ channel }) => mode === 'cli' && channel?.startsWith('chrome'), '--load-extension is not supported in Chrome anymore. https://groups.google.com/a/chromium.org/g/chromium-extensions/c/1-g8EFx2BBY/m/S0ET5wPjCAAJ');

    it('should return background pages', async ({ launchPersistentContext, asset }) => {
      const extensionPath = asset('extension-mv3-simple');
      const context = await launchPersistentContext(mode, extensionPath);
