...
for (const legacy of [true, false]) {
  it.describe('Drag and drop ' + (legacy ? 'legacy' : ''), () => {
    it.skip(({ isAndroid }) => isAndroid, 'No drag&drop on Android.');
    it.skip(({ mode }) => legacy && mode !== 'default', 'No env variables in out-of-process modes.');

    it.beforeAll(() => {
      if (legacy)
        process.env.PLAYWRIGHT_LEGACY_DRAG_AND_DROP = '1';
    });
    it.afterAll(() => {
      if (legacy)
        delete process.env.PLAYWRIGHT_LEGACY_DRAG_AND_DROP;
    });
    ...
    it('should not send dragover on the first mousemove', async ({ server, page, browserName }) => {
      it.fixme(!legacy, 'at least it is consistent between browsers!');
      it.fixme(legacy && browserName !== 'chromium');
    ...
    it('should cancel on escape', async ({ server, page, browserName }) => {
      ...
      expect(await events.jsonValue()).toEqual([
        ...
        browserName === 'firefox' ? 'mousemove at 240;350' : 'dragstart at 120;86',
        'dragenter at 240;350',
        legacy && browserName === 'chromium' ? null : 'dragover at 240;350',
        !legacy && browserName === 'chromium' ? 'dragleave at 240;350' : null,
        'dragend',
        'mouseup at 240;350',
      ].filter(Boolean));
    });
    ...
    it.describe('iframe', () => {
      it.fixme(({ browserName }) => legacy || browserName !== 'chromium', 'iframes dragging is not implemented');

      it('should drag into an iframe', async ({ server, page, isElectron }) => {
        it.fixme(isElectron);
