...
it('should dispatch boundary events when a re-appended node slides out from under the pointer', {
  annotation: { type: 'issue', description: 'https://github.com/microsoft/playwright/issues/42270' },
  ...
  await page.setContent(`
    <style>
      body { margin: 0 }
      div { width: 200px; color: white }
      #box { height: 90px; background: #b2334a }
      #spacer { height: 210px; background: #8a8a8a }
    </style>
    <div id=parent><div id=box>box</div><div id=spacer>spacer</div></div>
    <script>
      window.__log = [];
      const name = node => node.id || node.tagName.toLowerCase();
      for (const type of ['mouseover', 'mouseout'])
        document.addEventListener(type, e => window.__log.push(type + '[' + name(e.target) + ']'), true);
      // The d3 "move to front" idiom. Re-appending moves #box below #spacer, so the node ends up
      // out from under a pointer that never moved.
      const box = document.querySelector('#box');
      box.addEventListener('mouseover', () => box.parentNode.appendChild(box), { once: true });
      // Keep a lifecycle update pending so hover state is recomputed after the mutation.
      (function tick() { void document.body.offsetHeight; requestAnimationFrame(tick); })();
    </script>
  `);
  const log = () => page.evaluate(() => window['__log']);
  ...
  // The pointer has not moved, but #box has moved away from it, so it should be told it was left.
  // Chromium >= 144 does not restore the node's hover claim when it is re-attached, so #box never
  // gets its mouseout. See https://crbug.com/548251131.
  const expected = browserName === 'chromium'
    ? ['mouseover[box]', 'mouseover[spacer]']
    : ['mouseover[box]', 'mouseout[box]', 'mouseover[spacer]'];
  await expect.poll(log).toEqual(expected);
});
