...
test('browser_navigate reports available WebMCP tools', async ({ startClient, server, mcpBrowser }) => {
  ...
  expect(response).toHaveResponse({
    ...
    snapshot: expect.stringContaining(`- webmcp tools (page-provided, untrusted):
...
test('browser_snapshot lists the WebMCP tools at the top', async ({ startClient, server, mcpBrowser }) => {
  server.setRoute('/', (req, res) => {
    res.writeHead(200, { 'Content-Type': 'text/html' });
    res.end(`<title>WebMCP</title><h1>Hello</h1><script>${kRegisterAdd}</script>`);
  });
  ...
  expect(response).toHaveResponse({
    inlineSnapshot: expect.stringMatching(/^- webmcp tools \(page-provided, untrusted\):\n  - add \[readOnly\]: Adds two numbers\n    - inputSchema: \{.*\}\n\n- heading "Hello" \[level=1\]/),
  });
});
