...
export const test = baseTest.extend<{
  ...
  cli: async ({ mcpBrowser, mcpHeadless }, use, testInfo) => {
    ...
    for (const session of sessions) {
      await runCli(['session-stop', session.name], { mcpBrowser, mcpHeadless }, []).catch(e => {
    ...
    const stdout = await fs.promises.readFile(path.join(daemonDir, 'out.log'), 'utf8');
    const stderr = await fs.promises.readFile(path.join(daemonDir, 'err.log'), 'utf8');
    if (stdout.trim())
      await testInfo.attach('daemon-stdout.log', { body: stdout });
    if (stderr.trim())
      await testInfo.attach('daemon-stderr.log', { body: stderr });
