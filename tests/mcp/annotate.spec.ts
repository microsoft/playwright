...
test('should switch screencast to -s session on show --annotate', async ({ connectToDashboard, cli, mcpBrowser, server }) => {
  test.slow();
  test.fixme(mcpBrowser === 'firefox', 'Session-activation race: active session stays "first" after show --annotate. See https://github.com/microsoft/playwright/pull/42195');
  ...
  const browser = await connectToDashboard(bindTitle);
  const dashboard = browser.contexts()[0].pages()[0];
  await expect(dashboard.locator('#display')).toBeVisible({ timeout: 30_000 });
  ...
  await expect(dashboard.getByRole('main', { name: 'Dashboard: annotate' })).toBeVisible({ timeout: 15_000 });
  await expect(activeSession(dashboard)).toHaveAccessibleName('Session second', { timeout: 30_000 });
