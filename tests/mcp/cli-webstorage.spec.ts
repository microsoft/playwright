...
test('localstorage-set and localstorage-get', async ({ cli, server }, testInfo) => {
  ...
  // Set an item
  const { output: setOutput } = await cli('localstorage-set', 'testKey', 'testValue');
...
test('localstorage-delete removes item', async ({ cli, server }, testInfo) => {
  ...
  // Delete the item
  const { output: deleteOutput } = await cli('localstorage-delete', 'testKey');
...
test('localstorage-clear removes all items', async ({ cli, server }, testInfo) => {
  ...
  // Clear all items
  const { output: clearOutput } = await cli('localstorage-clear');
...
test('sessionstorage-set and sessionstorage-get', async ({ cli, server }, testInfo) => {
  ...
  // Set an item
  const { output: setOutput } = await cli('sessionstorage-set', 'testKey', 'testValue');
  expect(setOutput).toContain('set successfully');
...
test('sessionstorage-delete removes item', async ({ cli, server }, testInfo) => {
  ...
  // Delete the item
  const { output: deleteOutput } = await cli('sessionstorage-delete', 'testKey');
  expect(deleteOutput).toContain('deleted');
...
test('sessionstorage-clear removes all items', async ({ cli, server }, testInfo) => {
  ...
  // Clear all items
  const { output: clearOutput } = await cli('sessionstorage-clear');
  expect(clearOutput).toContain('cleared');
