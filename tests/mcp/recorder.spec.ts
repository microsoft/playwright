...
async function startHeadedClient(startClient: StartClient) {
  const { client } = await startClient({ omitArgs: ['--headless'] });
  return client;
}
...
test('record actions between start and stop', async ({ startClient, server }) => {
  const client = await startHeadedClient(startClient);
...
test('record navigation', async ({ startClient, server }) => {
  const client = await startHeadedClient(startClient);
  await navigateToForm(client, server);
...
test('stop with no actions recorded', async ({ startClient, server }) => {
  const client = await startHeadedClient(startClient);
  await navigateToForm(client, server);
  await client.callTool({ name: 'browser_start_recording' });
  expect(await client.callTool({
...
test('restarted recording only contains new actions', async ({ startClient, server }) => {
  const client = await startHeadedClient(startClient);
  server.setContent('/', `
    <title>Title</title>
    <button>Alpha</button>
...
test('actions performed while not recording are not included', async ({ startClient, server }) => {
  const client = await startHeadedClient(startClient);
  await navigateToForm(client, server);
...
test('record actions in python', async ({ startClient, server }) => {
  const { client } = await startClient({ args: ['--codegen=python'], omitArgs: ['--headless'] });
  await navigateToForm(client, server);
...
test('start twice is an error', async ({ startClient, server }) => {
  const client = await startHeadedClient(startClient);
  await navigateToForm(client, server);
  await client.callTool({ name: 'browser_start_recording' });
  expect(await client.callTool({
...
test('start in headless browser is an error', async ({ startClient }) => {
  const { client } = await startClient({ args: ['--headless'] });
  expect(await client.callTool({
    name: 'browser_start_recording',
  })).toHaveResponse({
    isError: true,
    error: expect.stringContaining('Recording user actions requires a headed browser'),
  });
});

test('stop without start is an error', async ({ startClient }) => {
  const client = await startHeadedClient(startClient);
  expect(await client.callTool({
    name: 'browser_stop_recording',
  })).toHaveResponse({
