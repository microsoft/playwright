...
test('cookie-list shows no cookies when empty', async ({ cli, server }, testInfo) => {
...
test('cookie-set and cookie-get', async ({ cli, server }, testInfo) => {
  ...
  // Set a cookie
  const { output: setOutput } = await cli('cookie-set', 'testCookie', 'testValue');
  expect(setOutput).toContain('set successfully');
...
test('cookie-delete removes cookie', async ({ cli, server }, testInfo) => {
  ...
  // Delete the cookie
  const { output: deleteOutput } = await cli('cookie-delete', 'testCookie');
...
test('cookie-clear removes all cookies', async ({ cli, server }, testInfo) => {
  ...
  // Clear all cookies
  const { output: clearOutput } = await cli('cookie-clear');
