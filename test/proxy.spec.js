...
describe('BrowserContext({proxy})', function() {
  ...
  it('should use proxy', async ({browser, server}) => {
    server.setRoute('/target.html', async (req, res) => {
      res.end('<html><title>Served by the proxy</title></html>');
    });
    const page = await browser.newPage({
  ...
  it('should authenticate', async ({browser, server}) => {
    server.setRoute('/target.html', async (req, res) => {
      const auth = req.headers['proxy-authorization'];
      if (!auth) {
        res.writeHead(407, 'Proxy Authentication Required', {
  ...
  it('should exclude patterns', async ({browser, server}) => {
    server.setRoute('/target.html', async (req, res) => {
      res.end('<html><title>Served by the proxy</title></html>');
    });
    const page = await browser.newPage({
  ...
  it('should use SOCKS proxy', async ({ browser, parallelIndex }) => {
    const server = socks.createServer((info, accept, deny) => {
      if (socket = accept(true)) {
        const body = '<html><title>Served by the SOCKS proxy</title></html>';
        socket.end([
