...
describe('Page.waitForLoadState', () => {
  ...
  it('should wait for load state of empty url popup', async ({browser, page}) => {
    ...
    await popup.waitForLoadState();
    expect(readyState).toBe(FIREFOX ? 'uninitialized' : 'complete');
    expect(await popup.evaluate(() => document.readyState)).toBe(FIREFOX ? 'uninitialized' : 'complete');
  });
...
describe('Page.goBack', function() {
  ...
  it.todo(WEBKIT && MAC)('should work for file urls', async ({page, server}) => {
    // WebKit embedder fails to go back/forward to the file url.
    const url1 = WIN
      ? 'file:///' + path.join(__dirname, 'assets', 'empty.html').replace(/\\/g, '/')
      : 'file://' + path.join(__dirname, 'assets', 'empty.html');
