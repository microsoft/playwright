...
export class TestServer {
  ...
  debugServer: any;
  _startTime: Date;
  _cachedPathPrefix: any;
  _sockets = new Set<Socket>();
  _routes: Map<any, any>;
  _auths: Map<any, any>;
  _csp: Map<any, any>;
  _gzipRoutes: Set<unknown>;
  _requestSubscribers: Map<any, any>;
  PORT: number;
  PREFIX: string;
  ...
  constructor(dirPath: string, port: number, sslOptions?: object) {
    if (sslOptions)
   ...
   */
  setRoute(path: string, handler: (arg0: http.IncomingMessage, arg1: http.ServerResponse) => void) {
