...
it('should record trace', async ({browserType, defaultBrowserOptions, server, tmpDir}) => {
  const artifactsPath = tmpDir;
  const browser = await browserType.launch({
    ...defaultBrowserOptions,
    artifactsPath,
  });
  const context = await browser.newContext({
    artifactsName: 'my-context',
    recordTrace: true,
  });
  ...
  const traceFile = path.join(artifactsPath, 'my-context', 'playwright.trace');
  ...
  expect(fs.existsSync(path.join(artifactsPath, '.sha1', gotoEvent.snapshot!.sha1))).toBe(true);
});
